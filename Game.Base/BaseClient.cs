    ...
    public class BaseClient
    {
		...
		private void RecvEventCallback(object sender, SocketAsyncEventArgs e)
		{
			try
			{
				...
				{
					IPEndPoint remoteIpEndPoint = (sender as Socket).RemoteEndPoint as IPEndPoint;

					if (remoteIpEndPoint != null && HydroFilter.IsBlocked(remoteIpEndPoint))
					{
						Console.Write("-");
						HydroFilter.BlockedPackets++;
						//Statistics.BlockedPackets++;
						Disconnect();
					}
					else
					{
						if (HydroFilter.IsActive)
						{
							...
							{
								HydroFilter.LogNewPacket(remoteIpEndPoint);
							}
						}
