        ...
        public static void SendStartMessage(BattleGame game)
        {
			...
			if (game.GameType != eGameType.EliteGameScore && game.GameType != eGameType.EliteGameChampion)
			{
				if (game.GameType == eGameType.Free)
				{
					...
					//game.SendToAll(pkg3);
					pkg1.WriteString(LanguageMgr.GetTranslation("Eþleþme bulundu! Özgür Savaþ baþlýyor..."));
				}
				else
				{
					pkg1.WriteString(LanguageMgr.GetTranslation("Eþleþme bulundu! Birlik Savaþý baþlýyor..."));
				}
			}
			else
			{
				pkg1.WriteString(LanguageMgr.GetTranslation("Eþleþme bulundu! Elit savaþ baþlýyor..."));
			}
