    ...
    public class ProxyRoomMgr
    {
        ...
        private static void PickUpRooms(long long_2)
        {
            ...
            foreach (ProxyRoom red in waitMatchRoomUnsafe)
            {
                    ...
                    switch (red.GameType)
                    {
                        case eGameType.Guild:
                            foreach (ProxyRoom current in waitMatchRoomUnsafe)
                            {
                                ...
                                {
                                    int guildID = GuildMgr.FindGuildRelationShip(red.GuildId, current.GuildId) + 1;
                                    int gameType = (int)current.GameType;
                                    int fightPower = Math.Abs(red.FightPower - current.FightPower);
                                    int svgLV = Math.Abs(red.AvgLevel - current.AvgLevel);
                                    int num6 = 10000;
                                    if (guildID * num6 + gameType * 1000 + fightPower + svgLV > minValue)
                                    {
                                        matchRoom = current;
                                        //break;
                                    }
                ...
                if (red.RoomType == eRoomType.ConsortiaBattle)
                {
                    foreach (ProxyRoom blue in waitMatchRoomUnsafe)
                    {
                        if (CanPickLocal(red, blue))
                        {
                            matchRoom = blue;
                        }
                    }
                ...
                if (!red.IsCrossZone)
                {//npc id yi nerede belirliyor
                    if (red.PickUpCount >= 4 && !red.startWithNpc && !red.isAutoBot && red.RoomType == eRoomType.Match && red.NpcId > 0 /*&& red.GameType != eGameType.Guild*/)
                    {
                    ...
                    {//debugu başlat görelim bi
                        ...
                        {
                            if (room != red && room.PlayerCount == red.PlayerCount && !room.IsPlaying && room.isAutoBot && red.NpcId == room.NpcId)
                            {
                                flag = true;
                                Console.WriteLine("Start fight with AutoBot No.{0}. RoomType: {1}, GameType: {2}", red.NpcId, red.RoomType, red.GameType);
                                StartMatchGame(red, room);
                                break;
                            }
                        }
                        if (!flag)
        ...
        public static bool CanPickLocal(ProxyRoom red, ProxyRoom blue)//, eRoomType redRoomType, int redPlayerCount)
        {
            if (blue != red && blue.RoomType == red.RoomType && !blue.IsPlaying && !blue.IsCrossZone && blue.ZoneId == red.ZoneId && blue.PlayerCount == red.PlayerCount)
            {
                return true;
            }
            return false;
