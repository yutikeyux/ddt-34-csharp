package bagAndInfo.bag
{
	...
	public class BagView extends Sprite
	{
		...
		protected function removeEvents() : void
		{
			...
			if(Boolean(this._bagLockControl))
			{
		...
		public function enableDressSelectedBtn(param1:Boolean) : void
		{
			...
			if(Boolean(this._trieveBtnSprite))
			{
				this._trieveBtnSprite.visible = param1;
			}
			if(Boolean(this._settedLockBtn))
			{
				this._settedLockBtn.visible = param1;
			}
			if(Boolean(this._settingLockBtn))
			{
				this._settingLockBtn.visible = param1;
			}
			if(Boolean(this._sortBagBtn))
			{
				this._sortBagBtn.visible = param1;
			}
			if(Boolean(this._keySetBtn))
			{
		...
		{
			if(Boolean(param1.changedProperties[PlayerInfo.MONEY]) || Boolean(param1.changedProperties[PlayerInfo.GOLD]) || Boolean(param1.changedProperties[PlayerInfo.MEDAL]) || Boolean(param1.changedProperties[PlayerInfo.GIFT]))
			{
				this.updateMoney();
			}
			else if(Boolean(param1.changedProperties["bagLocked"]))
			{
		...
		//bu formatnumber özelliği sayıları noktalı yapar not: yutikeyu
		...
		private function __cellUse(param1:Event) : void
		{
			...
			else if(_loc3_.info.TemplateID == EquipType.CHICKACTIVATION_CARDID)
			{
			...
			else if(_loc3_.info.TemplateID != EquipType.TRANSFER_PROP)
			{
				...
				else
				{
					...
					if(_loc3_.info.CategoryID == 11 && int(_loc3_.info.Property1) == 100)
					{
						this.useProp(_loc3_.itemInfo);
					}
					else
