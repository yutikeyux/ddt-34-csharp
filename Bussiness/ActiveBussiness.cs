...
namespace Bussiness
{
    public class ActiveBussiness : BaseCrossBussiness
    {
        ...
        public SubActiveInfo[] GetAllSubActive()
        {
            ...
            try
            {
                db.GetReader(ref reader, "SP_SubActive_All");
                while (reader.Read())
                {
                    SubActiveInfo info = new SubActiveInfo();
                    info.ID = (int)reader["ID"];
                    info.ActiveID = (int)reader["ActiveID"];
                    info.SubID = (int)reader["SubID"];
                    info.IsOpen = (bool)reader["IsOpen"];
                    info.StartDate = (DateTime)reader["StartDate"];
                    info.StartTime = (DateTime)reader["StartTime"];
                    info.EndDate = (DateTime)reader["EndDate"];
                    info.EndTime = (DateTime)reader["EndTime"];
                    info.IsContinued = (bool)reader["IsContinued"];
                    info.ActiveInfo = reader["ActiveInfo"] == null ? "" : reader["ActiveInfo"].ToString();
                    infos.Add(info);
                }
            }
            catch (Exception e)
        ...
        public SubActiveConditionInfo[] GetAllSubActiveCondition(int ActiveID)
        {
            ...
            try
            {
                ...
                db.GetReader(ref reader, "SP_SubActiveCondition_All", para);
                while (reader.Read())
                {
                    SubActiveConditionInfo info = new SubActiveConditionInfo();
                    info.ID = (int)reader["ID"];
                    info.ActiveID = (int)reader["ActiveID"];
                    info.SubID = (int)reader["SubID"];
                    info.ConditionID = (int)reader["ConditionID"];
                    info.Type = (int)reader["Type"];
                    info.Value = reader["Value"] == null ? "" : reader["Value"].ToString();
                    info.AwardType = (int)reader["AwardType"];
                    info.AwardValue = reader["AwardValue"] == null ? "" : reader["AwardValue"].ToString();
                    info.IsValid = (bool)reader["IsValid"];
                    infos.Add(info);
                }
            }
            catch (Exception e)
