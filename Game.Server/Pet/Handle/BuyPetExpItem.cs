...
namespace Game.Server.Pet.Handle
{
    ...
    public class BuyPetExpItem : IPetCommandHadler
    {
        public bool CommandHandler(GamePlayer player, GSPacketIn packet)
        {
            ...
            if (isBand)
            {
                if (player.MoneyDirect(needMoney, IsAntiMult: true, false, true))
                {
            ...
            else
            {
                ...
                {
                    if (needMoney < player.PlayerCharacter.Money || needMoney < player.PlayerCharacter.MoneyLock)
                    {
                        player.MoneyDirect(needMoney, true, false, true);
                        isContinue = true;
                    }
                }
            }
