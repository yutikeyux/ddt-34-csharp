...
namespace Game.Server.GameUtils
{
    public class PlayerActives
    {
        ...
        public bool IsPyramidOpen()
        {
            ...
            return DateTime.Now.Date < Convert.ToDateTime(GameProperties.PyramidEndTime).Date;
        }
        ...
        public void SendEvent()
        {
            ...
            if (GameMgr.GuildBattle.IsOpen/* && Player.CanActive("guildbattle")*/)
            {
            ...
            if (this.m_saveToDb)
            {
                using (PlayerBussiness playerBussiness = new PlayerBussiness())
                {
                    lock (this.m_lock)
                    {
                        ...
                        {
                            foreach (NewChickenBoxItemInfo chickenBoxReward in this.m_ChickenBoxRewards)
                            {
                                if (chickenBoxReward != null && chickenBoxReward.IsDirty)
                                {
                                    if (chickenBoxReward.ID > 0)
                                        playerBussiness.UpdateNewChickenBox(chickenBoxReward);
                                    else
                                        playerBussiness.AddNewChickenBox(chickenBoxReward);
                                }
                            }
                        }
                        if (this.m_RemoveChickenBoxRewards.Count > 0)
                        {
                            foreach (NewChickenBoxItemInfo chickenBoxReward in this.m_RemoveChickenBoxRewards)
                                playerBussiness.UpdateNewChickenBox(chickenBoxReward);
                        }
                    }
                }
