...
namespace Game.Server.GameUtils
{
    public class PlayerInventory : AbstractInventory
    {
        ...
        public const int EQUIPBAG_START = 31;

        public const int EQUIPBAG_END = 80;

        public const int DRESSBAG_START = 81;

        public const int DRESSBAG_END = 49 * 12;
            ...
            if (m_saveToDb)
            {
                using (PlayerBussiness pb = new PlayerBussiness())
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (item.ItemID == 3685862)
                            {
                                if (!AddItem(item, item.Place))
                                    overdueItems.Add(item);
                            }
                            else
                            {
                                if (BagType == (int)eBageType.EquipBag && Equip.isAvatar(item.Template) && item.Place > EQUIPBAG_START && item.Place < EQUIPBAG_END)
                                {
                                    int slot = FindFirstEmptySlot(DRESSBAG_START);
                                    if (slot >= 0)
                                    {
                            ...
                            //log.Info("LoadFromDatabase 2: " + item.ItemID + "|" + item.BagType + "|" + item.TemplateID);
                        }
                    }
                    catch (Exception ex)
                    {
                    ...
                    finally
                    {
                        ...
                        {
                            m_player.SendItemsToMail(overdueItems, null, LanguageMgr.GetTranslation("PlayerInventory.FullBag.Title"), eMailType.ItemOverdue);
