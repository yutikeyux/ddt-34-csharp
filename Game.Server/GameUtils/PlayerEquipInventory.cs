...
namespace Game.Server.GameUtils
{
    public class PlayerEquipInventory : PlayerInventory
    {
            ...
            try
            {
                ...
                lock (m_lock)
                {
                    ...
                    for (int i = 0; i < 31; i++)
                    {
                        ...
                        UserEquipGhostInfo ghostEquip = m_player.GetGhostEquip(item.BagType, item.Place);
                        if (ghostEquip != null)
                        {
                            SpiritInfo infoSpirit = SpiritInfoMgr.GetSingleSpirit(ghostEquip.BagType, ghostEquip.Place, ghostEquip.Level);
                            if (infoSpirit != null)
                            {
                                attack += item.Template.Attack * infoSpirit.AttackAdd / 1000;
                                agility += item.Template.Agility * infoSpirit.AgilityAdd / 1000;
                                defence += item.Template.Defence * infoSpirit.DefendAdd / 1000;
                                lucky += item.Template.Luck * infoSpirit.LuckAdd / 1000;
                            }
                        }
                    }
                    EquipBuffer();
                ...
                SetsBuildTempMgr.GetSetsBuildProp(m_player.PlayerCharacter.fineSuitExp, ref defence, ref hp, ref lucky, ref agility, ref damage);
        ...
        public void GetUserNimbus()
        {
            ...
            List<UserEquipGhostInfo> listEquipGhost = m_player.GetAllEquipGhost();
            foreach (UserEquipGhostInfo egInfo in listEquipGhost)
            {
