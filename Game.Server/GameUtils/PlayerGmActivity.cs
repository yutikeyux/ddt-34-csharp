...
namespace Game.Server.GameUtils
{
    public class PlayerGmActivity
    {
        ...
        {
            // DÜZELTME: Robot oyuncular için veritabanı yükleme işlemi yapılmamalıdır.
            // Robotların PlayerCharacter.ID'si veritabanında olmadığı için veya 
            // Client bağlantısı null olduğu için bu satırda hata oluşuyordu.
            if (m_player == null || m_player.PlayerCharacter == null || m_player.PlayerCharacter.IsAutoBot)
            {
                return;
            }
            ...
            using (PlayerBussiness playerBussiness = new PlayerBussiness())
            {
                ...
                // Mevcut aktivitelerden süresi geçenleri temizle
                ...
                if (GmActivityMgr.GmActivityInfos != null)
                {
                            ...
                            foreach (GmGiftInfo gift in list3)
                            {
                                ...
                                if (list8.Count == 0)
                                {
                                                                           ...
                                                                           })
                                    {
                                        ...
                                        {
                                            // DÜZELTME: Pet null ise hata vermemesi için kontrol
                                            if (m_player.Pet != null)
                                            {
                                                ...
                                                if (item3.GiftBagID == gift.giftbagId && list9 != null && list9.Count > 0 && list9[0].remain1 <= level)
                                                {
                                                    item3.Times = 1;
                                                }
                                            }
                                        ...
                                        else if (active.activityType == 20)
                                        {
                                            ...
                                            if (item3.GiftBagID == gift.giftbagId && list10 != null && list10.Count > 0 && list10[0].remain1 <= currentLevel)
                                            {
                                                item3.Times = 1;
                                            }
                                        }
