...
{
    public class MESblttacmongs1 : AMissionControl
    {
        ...
        private int redNpcID = 33050;

        private int blueNpcID = 33020;
        ...
        public override void OnNewTurnStarted() // mở
        {
            ...
            if (Game.TurnIndex > 1 && Game.GetLivedLivings().Count < 4)
            {
                ...
                if (redTotalCount < 25 && boss.IsLiving == true)
                {
                    redTotalCount++;
                    boss = Game.CreateBoss(blueNpcID, 1300, 444, -1, 10, "");
					boss.FallFrom(boss.X, boss.Y, "", 0, 0, 1000, null);
