    ...
    public class FLNS115 : AMissionControl
    {
        ...
        public void CreateNpc()
        {
			if (createNpcCount <= maxTurnSize)
			{
				...
				if (isCreateNpcFirst)
				{
					someNpc = base.Game.CreateNpc(redNpcID, 932, 769, 2, 1, config);
					isCreateNpcFirst = false;
					base.Game.WaitTime(0);
				}
				else
				{
					int x = base.Game.Random.Next(0, createNpcX.Length);
					someNpc = base.Game.CreateNpc(redNpcID, createNpcX[x].X, createNpcX[x].Y, 2, 1, config);
					createNpcCount++;
					currTurnID++;
					base.Game.WaitTime(0);
				}
