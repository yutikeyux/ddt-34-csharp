    ...
    public class FLSS104 : AMissionControl
    {
        ...
        public void CreateNpc()
        {
			if (createNpcCount <= maxTurnSize)
			{
				if (isCreateNpcFirst)
				{
					someNpc = base.Game.CreateNpc(redNpcID, 1075, 565, 2, -1, base.Game.BaseLivingConfig());
				...
				{
					someNpc = base.Game.CreateNpc(redNpcID, createNpcX[currTurnID], 565, 2, -1, base.Game.BaseLivingConfig());
					someNpc.Config.IsTurn = false;
					createNpcCount++;
					currTurnID++;
					base.Game.WaitTime(0);
