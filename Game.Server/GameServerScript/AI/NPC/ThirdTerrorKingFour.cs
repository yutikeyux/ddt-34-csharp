...
namespace GameServerScript.AI.NPC
{
	public class ThirdTerrorKingFour : ABrain
	{
		...
		public void HalfAttack()
		{
			...
			int num = base.Game.Random.Next(0, ThirdTerrorKingFour.SealChat.Length);
			base.Body.Say(ThirdTerrorKingFour.AllAttackChat[num], 1, 500);
		...
		private void PersonalAttackDame()
		{
			...
			int num = base.Game.Random.Next(0, ThirdTerrorKingFour.ShootChat.Length);
			((SimpleBoss)base.Body).Say(ThirdTerrorKingFour.ShootChat[num], 1, 500);
		...
		{
			int num = base.Game.Random.Next(0, ThirdTerrorKingFour.CallChat.Length);
			((SimpleBoss)base.Body).Say(ThirdTerrorKingFour.CallChat[num], 1, 500);
			if (this.IsEixt == 1)
			{
				base.Body.ChangeDirection(1, 50);
		...
		public void Summon()
		{
			...
			else
			{
				...
				Player player = base.Game.FindRandomPlayer();
				int num = base.Game.Random.Next(0, ThirdTerrorKingFour.KillPlayerChat.Length);
				((SimpleBoss)base.Body).Say(ThirdTerrorKingFour.KillPlayerChat[num], 1, 500);
		...
		public void MovePlayer()
		{
			...
			else
			{
				...
				Player player = base.Game.FindRandomPlayer();
				int num = base.Game.Random.Next(0, ThirdTerrorKingFour.SealChat.Length);
				((SimpleBoss)base.Body).Say(ThirdTerrorKingFour.SealChat[num], 1, 500);
				foreach (Player current in allLivingPlayers)
				{
		...
		public void KillAttack(int fx, int mx)
		{
			...
			int num = base.Game.Random.Next(0, ThirdTerrorKingFour.KillAttackChat.Length);
			((SimpleBoss)base.Body).Say(ThirdTerrorKingFour.KillAttackChat[num], 1, 500);
			base.Body.PlayMovie("beatC", 2500, 0);
