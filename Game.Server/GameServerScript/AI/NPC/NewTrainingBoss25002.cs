...
namespace GameServerScript.AI.NPC
{
	public class NewTrainingBoss25002 : ABrain
	{
		...
		public override void OnBeginNewTurn()
		{
			...
			{
				string oneChat = NewTrainingBoss25002.GetOneChat();
		...
		public void MoveToPlayer(Player player)
		{
			...
			if (num > 97)
			{
				if (num > ((SimpleBoss)base.Body).NpcInfo.MoveMax)
				{
					num = num2;
				}
				else
				...
				if (player.Y < 420 && player.X < 210)
				{
					if (base.Body.Y > 420)
					{
						base.Body.MoveTo(base.Body.X - num, base.Body.Y, "walk", 1200, "", ((SimpleBoss)base.Body).NpcInfo.speed, new LivingCallBack(this.MoveBeat));
					}
				...
				else if (base.Body.Y < 420)
				{
					...
					{
						base.Body.MoveTo(200, base.Body.Y, "walk", 1200, "", ((SimpleBoss)base.Body).NpcInfo.speed, new LivingCallBack(this.Fall));
					}
				}
				else if (player.X > base.Body.X)
				{
		...
		{
			base.Body.Beat(this.m_targer, "beatA", 100, 0, 200, 1, 1);
		}

		public void Beating()
		{
			if (this.m_targer != null && !base.Body.Beat(this.m_targer, "beatA", 100, 0, 200, 1, 1))
			{
		...
		public void Fall()
		{
			base.Body.FallFrom(base.Body.X, base.Body.Y + 240, null, 200, 0, 12, new LivingCallBack(this.Beating));
		}

		public static string GetOneChat()
		{
			int num = NewTrainingBoss25002.random.Next(0, NewTrainingBoss25002.listChat.Length);
			return NewTrainingBoss25002.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = NewTrainingBoss25002.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = NewTrainingBoss25002.random.Next(1, 3);
				}
				else
				{
					num = NewTrainingBoss25002.random.Next(1, 4);
				}
				if (num > 0)
				{
