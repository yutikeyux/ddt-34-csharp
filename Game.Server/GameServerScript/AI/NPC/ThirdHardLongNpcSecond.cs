...
namespace GameServerScript.AI.NPC
{
	public class ThirdHardLongNpcSecond : ABrain
	{
		...
		{
			int num = base.Game.Random.Next(0, ThirdHardLongNpcSecond.KillAttackChat.Length);
			base.Body.Say(ThirdHardLongNpcSecond.KillAttackChat[num], 1, 1000);
		...
		public override void OnKillPlayerSay()
		{
			...
			int num = base.Game.Random.Next(0, ThirdHardLongNpcSecond.KillPlayerChat.Length);
			base.Body.Say(ThirdHardLongNpcSecond.KillPlayerChat[num], 1, 0, 2000);
		}
		...
		{
			int num = base.Game.Random.Next(0, ThirdHardLongNpcSecond.ShootedChat.Length);
			if (this.isSay == 0 && base.Body.IsLiving)
			{
				base.Body.Say(ThirdHardLongNpcSecond.ShootedChat[num], 1, 900, 0);
				this.isSay = 1;
			}
			if (!base.Body.IsLiving)
			{
				num = base.Game.Random.Next(0, ThirdHardLongNpcSecond.DiedChat.Length);
				base.Body.Say(ThirdHardLongNpcSecond.DiedChat[num], 1, 100, 2000);
			}
		}

		public static string GetOneChat()
		{
			int num = ThirdHardLongNpcSecond.random.Next(0, ThirdHardLongNpcSecond.listChat.Length);
			return ThirdHardLongNpcSecond.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = ThirdHardLongNpcSecond.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = ThirdHardLongNpcSecond.random.Next(1, 3);
				}
				else
				{
					num = ThirdHardLongNpcSecond.random.Next(1, 4);
				}
				if (num > 0)
				{
