...
namespace GameServerScript.AI.NPC
{
	public class SeventhSimpleMaleAi : ABrain
	{
		...
		public override void OnBeginNewTurn()
		{
			...
			{
				string oneChat = SeventhSimpleMaleAi.GetOneChat();
		...
		{
			int num = SeventhSimpleMaleAi.random.Next(0, SeventhSimpleMaleAi.listChat.Length);
			return SeventhSimpleMaleAi.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = SeventhSimpleMaleAi.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = SeventhSimpleMaleAi.random.Next(1, 3);
				}
				else
				{
					num = SeventhSimpleMaleAi.random.Next(1, 4);
				}
				if (num > 0)
				{
