...
namespace GameServerScript.AI.NPC
{
	public class ThirdNormalShortNpcFirst : ABrain
	{
		...
		public override void OnBeginNewTurn()
		{
			...
			{
				string oneChat = ThirdNormalShortNpcFirst.GetOneChat();
		...
		{
			int num = ThirdNormalShortNpcFirst.random.Next(0, ThirdNormalShortNpcFirst.listChat.Length);
			return ThirdNormalShortNpcFirst.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = ThirdNormalShortNpcFirst.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = ThirdNormalShortNpcFirst.random.Next(1, 3);
				}
				else
				{
					num = ThirdNormalShortNpcFirst.random.Next(1, 4);
				}
				if (num > 0)
				{
