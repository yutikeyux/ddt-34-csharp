...
namespace GameServerScript.AI.NPC
{
	public class ThirdHardLongNpcFirst : ABrain
	{
		...
		{
			int num = base.Game.Random.Next(0, ThirdHardLongNpcFirst.KillAttackChat.Length);
			base.Body.Say(ThirdHardLongNpcFirst.KillAttackChat[num], 0, 1000);
			base.Body.CurrentDamagePlus = 1f;
		...
		public override void OnKillPlayerSay()
		{
			...
			int num = base.Game.Random.Next(0, ThirdHardLongNpcFirst.KillPlayerChat.Length);
			base.Body.Say(ThirdHardLongNpcFirst.KillPlayerChat[num], 1, 0, 2000);
		}
		...
		{
			int num = base.Game.Random.Next(0, ThirdHardLongNpcFirst.ShootedChat.Length);
			if (this.isSay == 0 && base.Body.IsLiving)
			{
				base.Body.Say(ThirdHardLongNpcFirst.ShootedChat[num], 1, 900, 0);
				this.isSay = 1;
			}
			if (!base.Body.IsLiving)
			{
				num = base.Game.Random.Next(0, ThirdHardLongNpcFirst.DiedChat.Length);
				base.Body.Say(ThirdHardLongNpcFirst.DiedChat[num], 1, 100, 2000);
			}
		}

		public static string GetOneChat()
		{
			int num = ThirdHardLongNpcFirst.random.Next(0, ThirdHardLongNpcFirst.listChat.Length);
			return ThirdHardLongNpcFirst.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = ThirdHardLongNpcFirst.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = ThirdHardLongNpcFirst.random.Next(1, 3);
				}
				else
				{
					num = ThirdHardLongNpcFirst.random.Next(1, 4);
				}
				if (num > 0)
				{
