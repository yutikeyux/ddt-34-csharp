...
namespace GameServerScript.AI.NPC
{
	public class ThirdSimpleKingFour : ABrain
	{
		...
		public void HalfAttack()
		{
			...
			int num = base.Game.Random.Next(0, ThirdSimpleKingFour.SealChat.Length);
			base.Body.Say(ThirdSimpleKingFour.AllAttackChat[num], 1, 500);
		...
		private void PersonalAttackDame()
		{
			...
			int num = base.Game.Random.Next(0, ThirdSimpleKingFour.ShootChat.Length);
			((SimpleBoss)base.Body).Say(ThirdSimpleKingFour.ShootChat[num], 1, 500);
		...
		{
			int num = base.Game.Random.Next(0, ThirdSimpleKingFour.CallChat.Length);
			((SimpleBoss)base.Body).Say(ThirdSimpleKingFour.CallChat[num], 1, 500);
			if (this.IsEixt == 1)
			{
				base.Body.ChangeDirection(1, 50);
		...
		public void Summon()
		{
			...
			else
			{
				...
				Player player = base.Game.FindRandomPlayer();
				int num = base.Game.Random.Next(0, ThirdSimpleKingFour.KillPlayerChat.Length);
				((SimpleBoss)base.Body).Say(ThirdSimpleKingFour.KillPlayerChat[num], 1, 500);
		...
		public void MovePlayer()
		{
			...
			else
			{
				...
				Player player = base.Game.FindRandomPlayer();
				int num = base.Game.Random.Next(0, ThirdSimpleKingFour.SealChat.Length);
				((SimpleBoss)base.Body).Say(ThirdSimpleKingFour.SealChat[num], 1, 500);
				foreach (Player current in allLivingPlayers)
				{
		...
		public void KillAttack(int fx, int mx)
		{
			...
			int num = base.Game.Random.Next(0, ThirdSimpleKingFour.KillAttackChat.Length);
			((SimpleBoss)base.Body).Say(ThirdSimpleKingFour.KillAttackChat[num], 1, 500);
			base.Body.PlayMovie("beatC", 2500, 0);
