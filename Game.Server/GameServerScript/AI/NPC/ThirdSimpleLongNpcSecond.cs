...
namespace GameServerScript.AI.NPC
{
	public class ThirdSimpleLongNpcSecond : ABrain
	{
		...
		{
			int num = base.Game.Random.Next(0, ThirdSimpleLongNpcSecond.KillAttackChat.Length);
			base.Body.Say(ThirdSimpleLongNpcSecond.KillAttackChat[num], 1, 1000);
		...
		private void NextAttack()
		{
			...
			if (player != null)
			{
				...
				if (base.Body.ShootPoint(1194, player.Y, 58, 1000, 10000, 1, 3f, 2550))
				{
		...
		public override void OnKillPlayerSay()
		{
			...
			int num = base.Game.Random.Next(0, ThirdSimpleLongNpcSecond.KillPlayerChat.Length);
			base.Body.Say(ThirdSimpleLongNpcSecond.KillPlayerChat[num], 1, 0, 2000);
		}
		...
		{
			int num = base.Game.Random.Next(0, ThirdSimpleLongNpcSecond.ShootedChat.Length);
			if (this.isSay == 0 && base.Body.IsLiving)
			{
				base.Body.Say(ThirdSimpleLongNpcSecond.ShootedChat[num], 1, 900, 0);
				this.isSay = 1;
			}
			if (!base.Body.IsLiving)
			{
				num = base.Game.Random.Next(0, ThirdSimpleLongNpcSecond.DiedChat.Length);
				base.Body.Say(ThirdSimpleLongNpcSecond.DiedChat[num], 1, 100, 2000);
			}
		}

		public static string GetOneChat()
		{
			int num = ThirdSimpleLongNpcSecond.random.Next(0, ThirdSimpleLongNpcSecond.listChat.Length);
			return ThirdSimpleLongNpcSecond.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = ThirdSimpleLongNpcSecond.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = ThirdSimpleLongNpcSecond.random.Next(1, 3);
				}
				else
				{
					num = ThirdSimpleLongNpcSecond.random.Next(1, 4);
				}
				if (num > 0)
				{
