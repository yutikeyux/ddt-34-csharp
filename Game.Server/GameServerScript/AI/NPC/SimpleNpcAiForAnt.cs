...
namespace GameServerScript.AI.NPC
{
	public class SimpleNpcAiForAnt : ABrain
	{
		...
		public override void OnBeginNewTurn()
		{
			...
			{
				string oneChat = SimpleNpcAiForAnt.GetOneChat();
		...
		{
			int num = SimpleNpcAiForAnt.random.Next(0, SimpleNpcAiForAnt.listChat.Length);
			return SimpleNpcAiForAnt.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = SimpleNpcAiForAnt.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = SimpleNpcAiForAnt.random.Next(1, 3);
				}
				else
				{
					num = SimpleNpcAiForAnt.random.Next(1, 4);
				}
				if (num > 0)
				{
