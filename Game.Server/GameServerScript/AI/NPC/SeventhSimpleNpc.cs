...
namespace GameServerScript.AI.NPC
{
	public class SeventhSimpleNpc : ABrain
	{
		...
		public override void OnBeginNewTurn()
		{
			...
			{
				string oneChat = SeventhSimpleNpc.GetOneChat();
		...
		{
			int num = SeventhSimpleNpc.random.Next(0, SeventhSimpleNpc.listChat.Length);
			return SeventhSimpleNpc.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = SeventhSimpleNpc.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = SeventhSimpleNpc.random.Next(1, 3);
				}
				else
				{
					num = SeventhSimpleNpc.random.Next(1, 4);
				}
				if (num > 0)
				{
