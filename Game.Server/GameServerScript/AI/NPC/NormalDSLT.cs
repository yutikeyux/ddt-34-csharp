...
{
	public class NormalDSLT : ABrain
	{
		private int m_attackTurn = 0;

		private int npcID = 71123;
		...
		{
			int num = base.Game.Random.Next(0, NormalDSLT.KillAttackChat.Length);
			base.Body.Say(NormalDSLT.KillAttackChat[num], 1, 1000);
		...
		private void PersonalAttack()
		{
			...
			{
				base.Body.CurrentDamagePlus = 1f;
				int num = base.Game.Random.Next(0, NormalDSLT.ShootChat.Length);
				base.Body.Say(NormalDSLT.ShootChat[num], 1, 0);
		...
		private void Summon()
		{
			int num = base.Game.Random.Next(0, NormalDSLT.CallChat.Length);
			base.Body.Say(NormalDSLT.CallChat[num], 1, 600);
		...
		public override void OnKillPlayerSay()
		{
			...
			int num = base.Game.Random.Next(0, NormalDSLT.KillPlayerChat.Length);
			base.Body.Say(NormalDSLT.KillPlayerChat[num], 1, 0, 2000);
		}
		...
		{
			int num = base.Game.Random.Next(0, NormalDSLT.ShootedChat.Length);
			if (this.isSay == 0 && base.Body.IsLiving)
			{
				base.Body.Say(NormalDSLT.ShootedChat[num], 1, 900, 0);
				this.isSay = 1;
			}
			if (!base.Body.IsLiving)
			{
				num = base.Game.Random.Next(0, NormalDSLT.DiedChat.Length);
				base.Body.Say(NormalDSLT.DiedChat[num], 1, 100, 2000);
			}
		}
	}
}
