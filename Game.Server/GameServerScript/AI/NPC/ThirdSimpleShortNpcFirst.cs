...
namespace GameServerScript.AI.NPC
{
	public class ThirdSimpleShortNpcFirst : ABrain
	{
		...
		public override void OnBeginNewTurn()
		{
			...
			{
				string oneChat = ThirdSimpleShortNpcFirst.GetOneChat();
		...
		{
			int num = ThirdSimpleShortNpcFirst.random.Next(0, ThirdSimpleShortNpcFirst.listChat.Length);
			return ThirdSimpleShortNpcFirst.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = ThirdSimpleShortNpcFirst.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = ThirdSimpleShortNpcFirst.random.Next(1, 3);
				}
				else
				{
					num = ThirdSimpleShortNpcFirst.random.Next(1, 4);
				}
				if (num > 0)
				{
