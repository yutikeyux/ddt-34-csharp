...
namespace GameServerScript.AI.NPC
{
	public class SeventhHardNpc : ABrain
	{
		...
		public override void OnBeginNewTurn()
		{
			...
			{
				string oneChat = SeventhHardNpc.GetOneChat();
		...
		{
			int num = SeventhHardNpc.random.Next(0, SeventhHardNpc.listChat.Length);
			return SeventhHardNpc.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = SeventhHardNpc.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = SeventhHardNpc.random.Next(1, 3);
				}
				else
				{
					num = SeventhHardNpc.random.Next(1, 4);
				}
				if (num > 0)
				{
