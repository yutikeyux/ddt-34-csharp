...
namespace GameServerScript.AI.NPC
{
	public class ThirdNormalFixureLongNpc : ABrain
	{
		...
		{
			int num = base.Game.Random.Next(0, ThirdNormalFixureLongNpc.KillAttackChat.Length);
			base.Body.Say(ThirdNormalFixureLongNpc.KillAttackChat[num], 1, 1000);
		...
		public override void OnKillPlayerSay()
		{
			...
			int num = base.Game.Random.Next(0, ThirdNormalFixureLongNpc.KillPlayerChat.Length);
			base.Body.Say(ThirdNormalFixureLongNpc.KillPlayerChat[num], 1, 0, 2000);
		}
		...
		{
			int num = base.Game.Random.Next(0, ThirdNormalFixureLongNpc.ShootedChat.Length);
			if (this.isSay == 0 && base.Body.IsLiving)
			{
				base.Body.Say(ThirdNormalFixureLongNpc.ShootedChat[num], 1, 900, 0);
				this.isSay = 1;
			}
			if (!base.Body.IsLiving)
			{
				num = base.Game.Random.Next(0, ThirdNormalFixureLongNpc.DiedChat.Length);
				base.Body.Say(ThirdNormalFixureLongNpc.DiedChat[num], 1, 100, 2000);
			}
		}

		public static string GetOneChat()
		{
			int num = ThirdNormalFixureLongNpc.random.Next(0, ThirdNormalFixureLongNpc.listChat.Length);
			return ThirdNormalFixureLongNpc.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = ThirdNormalFixureLongNpc.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = ThirdNormalFixureLongNpc.random.Next(1, 3);
				}
				else
				{
					num = ThirdNormalFixureLongNpc.random.Next(1, 4);
				}
				if (num > 0)
				{
