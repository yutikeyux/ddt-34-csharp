...
namespace GameServerScript.AI.NPC
{
	public class ThirdTerrorLongNpcSecond : ABrain
	{
		...
		{
			int num = base.Game.Random.Next(0, ThirdTerrorLongNpcSecond.KillAttackChat.Length);
			base.Body.Say(ThirdTerrorLongNpcSecond.KillAttackChat[num], 1, 1000);
		...
		public override void OnKillPlayerSay()
		{
			...
			int num = base.Game.Random.Next(0, ThirdTerrorLongNpcSecond.KillPlayerChat.Length);
			base.Body.Say(ThirdTerrorLongNpcSecond.KillPlayerChat[num], 1, 0, 2000);
		}
		...
		{
			int num = base.Game.Random.Next(0, ThirdTerrorLongNpcSecond.ShootedChat.Length);
			if (this.isSay == 0 && base.Body.IsLiving)
			{
				base.Body.Say(ThirdTerrorLongNpcSecond.ShootedChat[num], 1, 900, 0);
				this.isSay = 1;
			}
			if (!base.Body.IsLiving)
			{
				num = base.Game.Random.Next(0, ThirdTerrorLongNpcSecond.DiedChat.Length);
				base.Body.Say(ThirdTerrorLongNpcSecond.DiedChat[num], 1, 100, 2000);
			}
		}

		public static string GetOneChat()
		{
			int num = ThirdTerrorLongNpcSecond.random.Next(0, ThirdTerrorLongNpcSecond.listChat.Length);
			return ThirdTerrorLongNpcSecond.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = ThirdTerrorLongNpcSecond.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = ThirdTerrorLongNpcSecond.random.Next(1, 3);
				}
				else
				{
					num = ThirdTerrorLongNpcSecond.random.Next(1, 4);
				}
				if (num > 0)
				{
