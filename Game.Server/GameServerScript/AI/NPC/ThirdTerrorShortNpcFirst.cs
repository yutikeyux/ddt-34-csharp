...
namespace GameServerScript.AI.NPC
{
	public class ThirdTerrorShortNpcFirst : ABrain
	{
		...
		public override void OnBeginNewTurn()
		{
			...
			{
				string oneChat = ThirdTerrorShortNpcFirst.GetOneChat();
		...
		{
			int num = ThirdTerrorShortNpcFirst.random.Next(0, ThirdTerrorShortNpcFirst.listChat.Length);
			return ThirdTerrorShortNpcFirst.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = ThirdTerrorShortNpcFirst.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = ThirdTerrorShortNpcFirst.random.Next(1, 3);
				}
				else
				{
					num = ThirdTerrorShortNpcFirst.random.Next(1, 4);
				}
				if (num > 0)
				{
