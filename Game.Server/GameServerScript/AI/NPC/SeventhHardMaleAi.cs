...
namespace GameServerScript.AI.NPC
{
	public class SeventhHardMaleAi : ABrain
	{
		...
		public override void OnBeginNewTurn()
		{
			...
			{
				string oneChat = SeventhHardMaleAi.GetOneChat();
		...
		{
			int num = SeventhHardMaleAi.random.Next(0, SeventhHardMaleAi.listChat.Length);
			return SeventhHardMaleAi.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = SeventhHardMaleAi.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = SeventhHardMaleAi.random.Next(1, 3);
				}
				else
				{
					num = SeventhHardMaleAi.random.Next(1, 4);
				}
				if (num > 0)
				{
