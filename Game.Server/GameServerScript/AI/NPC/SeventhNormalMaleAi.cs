...
namespace GameServerScript.AI.NPC
{
	public class SeventhNormalMaleAi : ABrain
	{
		...
		public override void OnBeginNewTurn()
		{
			...
			{
				string oneChat = SeventhNormalMaleAi.GetOneChat();
		...
		{
			int num = SeventhNormalMaleAi.random.Next(0, SeventhNormalMaleAi.listChat.Length);
			return SeventhNormalMaleAi.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = SeventhNormalMaleAi.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = SeventhNormalMaleAi.random.Next(1, 3);
				}
				else
				{
					num = SeventhNormalMaleAi.random.Next(1, 4);
				}
				if (num > 0)
				{
