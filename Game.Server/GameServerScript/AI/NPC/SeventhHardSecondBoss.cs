...
namespace GameServerScript.AI.NPC
{
	public class SeventhHardSecondBoss : ABrain
	{
		...
		{
			int num = base.Game.Random.Next(0, SeventhHardSecondBoss.KillAttackChat.Length);
			base.Body.Say(SeventhHardSecondBoss.KillAttackChat[num], 1, 0);
		...
		private void AttackingB()
		{
			...
			if (player != null)
			{
				base.Body.MoveTo(player.X - 150, player.Y, "run", 1000, "", 18, new LivingCallBack(this.NextAttackB));
			}
		}
		...
		{
			base.Body.Direction = base.Game.FindlivingbyDir(base.Body);
			base.Body.MoveTo(181, base.Body.Y, "run", 1000, "", 18, new LivingCallBack(this.ChangeDirection));
		}
		...
		private void AttackingA()
		{
			...
			if (player != null)
			{
				base.Body.MoveTo(player.X - 150, player.Y, "run", 1000, "", 18, new LivingCallBack(this.NextAttackA));
			}
		}
		...
		{
			base.Body.MoveTo(1477, base.Body.Y, "run", 1000, "", 18, new LivingCallBack(this.PersonalAttack));
		}
		...
		private void GoMovie()
		{
			...
			if (player != null)
			{
				...
				base.Body.CallFuction(new LivingCallBack(this.GoAttacking), 3000);
			}
		}
		...
		private void GoAttacking()
		{
			...
			if (!this.IsEixt && player != null)
			{
				...
				foreach (Player current in allLivingPlayers)
				{
					...
					{
						current.AddEffect(new ContinueReduceBlood(2, 900, current), 0);
					}
				}
			}
