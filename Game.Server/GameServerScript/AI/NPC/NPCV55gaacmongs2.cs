...
{
	public class NPCV55gaacmongs2 : ABrain
	{
		...
		public override void OnStartAttacking()
		{
			...
			{
				if (current.IsLiving && current.X > 770)
				{
			...
			if (flag)
			{
				...
				base.Body.Say(NPCV55gaacmongs2.AttackAChat[1], 1, 1000);				
			}
			else if (this.m_attackTurn == 0)
			{
				this.AttackA();
				base.Body.Say(NPCV55gaacmongs2.AttackAChat[2], 1, 1000);				
				this.m_attackTurn++;
			}
			else if (this.m_attackTurn == 1)
			{
				this.AttackB();
				base.Body.Say(NPCV55gaacmongs2.AttackAChat[3], 1, 1000);				
				this.m_attackTurn++;
			}
			else
			{
				this.AttackC();
				base.Body.Say(NPCV55gaacmongs2.AttackAChat[4], 1, 1000);				
				this.m_attackTurn = 0;
			}
		}
