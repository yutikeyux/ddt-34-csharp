...
namespace GameServerScript.AI.NPC
{
	public class SeventhNormalSecondBoss : ABrain
	{
		...
		{
			int num = base.Game.Random.Next(0, SeventhNormalSecondBoss.KillAttackChat.Length);
			base.Body.Say(SeventhNormalSecondBoss.KillAttackChat[num], 1, 0);
		...
		private void GoAttacking()
		{
			...
			if (!this.IsEixt && player != null)
			{
				...
				foreach (Player current in allLivingPlayers)
				{
					...
					{
						current.AddEffect(new ContinueReduceBlood(2, 500, current), 0);
					}
				}
			}
