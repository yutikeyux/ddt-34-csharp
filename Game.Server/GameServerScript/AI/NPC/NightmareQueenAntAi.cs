...
namespace GameServerScript.AI.NPC
{
	public class NightmareQueenAntAi : ABrain
	{
		...
		{
			int num = base.Game.Random.Next(0, NightmareQueenAntAi.KillAttackChat.Length);
			base.Body.Say(NightmareQueenAntAi.KillAttackChat[num], 1, 1000);
		...
		private void PersonalAttack()
		{
			...
			if (player != null)
			{
				...
				int num = base.Game.Random.Next(0, NightmareQueenAntAi.ShootChat.Length);
				base.Body.Say(NightmareQueenAntAi.ShootChat[num], 1, 0);
		...
		private void Summon()
		{
			int num = base.Game.Random.Next(0, NightmareQueenAntAi.CallChat.Length);
			base.Body.Say(NightmareQueenAntAi.CallChat[num], 1, 600);
		...
		{
			int num = base.Game.Random.Next(0, NightmareQueenAntAi.ShootedChat.Length);
			if (this.isSay == 0 && base.Body.IsLiving)
			{
				base.Body.Say(NightmareQueenAntAi.ShootedChat[num], 1, delay, 0);
				this.isSay = 1;
			}
			if (!base.Body.IsLiving)
			{
				num = base.Game.Random.Next(0, NightmareQueenAntAi.DiedChat.Length);
				base.Body.Say(NightmareQueenAntAi.DiedChat[num], 1, delay - 800, 2000);
			}
		}
	}
}
