...
namespace GameServerScript.AI.NPC
{
	public class ThirdTerrorLongNpcFirst : ABrain
	{
		...
		{
			int num = base.Game.Random.Next(0, ThirdTerrorLongNpcFirst.KillAttackChat.Length);
			base.Body.Say(ThirdTerrorLongNpcFirst.KillAttackChat[num], 0, 1000);
			base.Body.CurrentDamagePlus = 1f;
		...
		private void PersonalAttack()
		{
			...
			base.Body.MoveTo(base.Body.X + num, base.Body.Y, "walk", 1000, "", 3, new LivingCallBack(this.NextAttack));
		}
		...
		public override void OnKillPlayerSay()
		{
			...
			int num = base.Game.Random.Next(0, ThirdTerrorLongNpcFirst.KillPlayerChat.Length);
			base.Body.Say(ThirdTerrorLongNpcFirst.KillPlayerChat[num], 1, 0, 2000);
		}
		...
		{
			int num = base.Game.Random.Next(0, ThirdTerrorLongNpcFirst.ShootedChat.Length);
			if (this.isSay == 0 && base.Body.IsLiving)
			{
				base.Body.Say(ThirdTerrorLongNpcFirst.ShootedChat[num], 1, 900, 0);
				this.isSay = 1;
			}
			if (!base.Body.IsLiving)
			{
				num = base.Game.Random.Next(0, ThirdTerrorLongNpcFirst.DiedChat.Length);
				base.Body.Say(ThirdTerrorLongNpcFirst.DiedChat[num], 1, 100, 2000);
			}
		}

		public static string GetOneChat()
		{
			int num = ThirdTerrorLongNpcFirst.random.Next(0, ThirdTerrorLongNpcFirst.listChat.Length);
			return ThirdTerrorLongNpcFirst.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = ThirdTerrorLongNpcFirst.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = ThirdTerrorLongNpcFirst.random.Next(1, 3);
				}
				else
				{
					num = ThirdTerrorLongNpcFirst.random.Next(1, 4);
				}
				if (num > 0)
				{
