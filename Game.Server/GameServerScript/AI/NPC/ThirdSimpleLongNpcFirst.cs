...
namespace GameServerScript.AI.NPC
{
	public class ThirdSimpleLongNpcFirst : ABrain
	{
		...
		{
			int num = base.Game.Random.Next(0, ThirdSimpleLongNpcFirst.KillAttackChat.Length);
			base.Body.Say(ThirdSimpleLongNpcFirst.KillAttackChat[num], 0, 1000);
			base.Body.CurrentDamagePlus = 1f;
		...
		public override void OnKillPlayerSay()
		{
			...
			int num = base.Game.Random.Next(0, ThirdSimpleLongNpcFirst.KillPlayerChat.Length);
			base.Body.Say(ThirdSimpleLongNpcFirst.KillPlayerChat[num], 1, 0, 2000);
		}
		...
		{
			int num = base.Game.Random.Next(0, ThirdSimpleLongNpcFirst.ShootedChat.Length);
			if (this.isSay == 0 && base.Body.IsLiving)
			{
				base.Body.Say(ThirdSimpleLongNpcFirst.ShootedChat[num], 1, 900, 0);
				this.isSay = 1;
			}
			if (!base.Body.IsLiving)
			{
				num = base.Game.Random.Next(0, ThirdSimpleLongNpcFirst.DiedChat.Length);
				base.Body.Say(ThirdSimpleLongNpcFirst.DiedChat[num], 1, 100, 2000);
			}
		}

		public static string GetOneChat()
		{
			int num = ThirdSimpleLongNpcFirst.random.Next(0, ThirdSimpleLongNpcFirst.listChat.Length);
			return ThirdSimpleLongNpcFirst.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = ThirdSimpleLongNpcFirst.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = ThirdSimpleLongNpcFirst.random.Next(1, 3);
				}
				else
				{
					num = ThirdSimpleLongNpcFirst.random.Next(1, 4);
				}
				if (num > 0)
				{
