...
namespace GameServerScript.AI.NPC
{
	public class ThirdHardFixureLongNpc : ABrain
	{
		...
		{
			int num = base.Game.Random.Next(0, ThirdHardFixureLongNpc.KillAttackChat.Length);
			base.Body.Say(ThirdHardFixureLongNpc.KillAttackChat[num], 1, 1000);
		...
		public override void OnKillPlayerSay()
		{
			...
			int num = base.Game.Random.Next(0, ThirdHardFixureLongNpc.KillPlayerChat.Length);
			base.Body.Say(ThirdHardFixureLongNpc.KillPlayerChat[num], 1, 0, 2000);
		}
		...
		{
			int num = base.Game.Random.Next(0, ThirdHardFixureLongNpc.ShootedChat.Length);
			if (this.isSay == 0 && base.Body.IsLiving)
			{
				base.Body.Say(ThirdHardFixureLongNpc.ShootedChat[num], 1, 900, 0);
				this.isSay = 1;
			}
			if (!base.Body.IsLiving)
			{
				num = base.Game.Random.Next(0, ThirdHardFixureLongNpc.DiedChat.Length);
				base.Body.Say(ThirdHardFixureLongNpc.DiedChat[num], 1, 100, 2000);
			}
		}

		public static string GetOneChat()
		{
			int num = ThirdHardFixureLongNpc.random.Next(0, ThirdHardFixureLongNpc.listChat.Length);
			return ThirdHardFixureLongNpc.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = ThirdHardFixureLongNpc.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = ThirdHardFixureLongNpc.random.Next(1, 3);
				}
				else
				{
					num = ThirdHardFixureLongNpc.random.Next(1, 4);
				}
				if (num > 0)
				{
