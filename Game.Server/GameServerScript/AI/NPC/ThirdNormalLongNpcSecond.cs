...
namespace GameServerScript.AI.NPC
{
	public class ThirdNormalLongNpcSecond : ABrain
	{
		...
		{
			int num = base.Game.Random.Next(0, ThirdNormalLongNpcSecond.KillAttackChat.Length);
			base.Body.Say(ThirdNormalLongNpcSecond.KillAttackChat[num], 1, 1000);
		...
		public override void OnKillPlayerSay()
		{
			...
			int num = base.Game.Random.Next(0, ThirdNormalLongNpcSecond.KillPlayerChat.Length);
			base.Body.Say(ThirdNormalLongNpcSecond.KillPlayerChat[num], 1, 0, 2000);
		}
		...
		{
			int num = base.Game.Random.Next(0, ThirdNormalLongNpcSecond.ShootedChat.Length);
			if (this.isSay == 0 && base.Body.IsLiving)
			{
				base.Body.Say(ThirdNormalLongNpcSecond.ShootedChat[num], 1, 900, 0);
				this.isSay = 1;
			}
			if (!base.Body.IsLiving)
			{
				num = base.Game.Random.Next(0, ThirdNormalLongNpcSecond.DiedChat.Length);
				base.Body.Say(ThirdNormalLongNpcSecond.DiedChat[num], 1, 100, 2000);
			}
		}

		public static string GetOneChat()
		{
			int num = ThirdNormalLongNpcSecond.random.Next(0, ThirdNormalLongNpcSecond.listChat.Length);
			return ThirdNormalLongNpcSecond.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = ThirdNormalLongNpcSecond.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = ThirdNormalLongNpcSecond.random.Next(1, 3);
				}
				else
				{
					num = ThirdNormalLongNpcSecond.random.Next(1, 4);
				}
				if (num > 0)
				{
