...
namespace GameServerScript.AI.NPC
{
	public class SimpleNpcAi : ABrain
	{
		...
		{
			int num = SimpleNpcAi.random.Next(0, SimpleNpcAi.listChat.Length);
			return SimpleNpcAi.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = SimpleNpcAi.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = SimpleNpcAi.random.Next(1, 3);
				}
				else
				{
					num = SimpleNpcAi.random.Next(1, 4);
				}
				if (num > 0)
				{
