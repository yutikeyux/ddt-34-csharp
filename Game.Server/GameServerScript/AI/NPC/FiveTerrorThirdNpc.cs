...
namespace GameServerScript.AI.NPC
{
	public class FiveTerrorThirdNpc : ABrain
	{
		...
		public override void OnBeginNewTurn()
		{
			...
			{
				string oneChat = FiveTerrorThirdNpc.GetOneChat();
		...
		{
			int num = FiveTerrorThirdNpc.random.Next(0, FiveTerrorThirdNpc.listChat.Length);
			return FiveTerrorThirdNpc.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = FiveTerrorThirdNpc.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = FiveTerrorThirdNpc.random.Next(1, 3);
				}
				else
				{
					num = FiveTerrorThirdNpc.random.Next(1, 4);
				}
				if (num > 0)
				{
					...
					while (i < num)
					{
						...
						if (!livings[index].IsSay)
						{
							...
							int num2 = FiveTerrorThirdNpc.random.Next(0, 5000);
							i++;
						}
					}
				}
