...
namespace GameServerScript.AI.NPC
{
	public class ThirteenHardBrynBoss : ABrain
	{
		...
		{
			int num = base.Game.Random.Next(0, ThirteenHardBrynBoss.KillAttackChat.Length);
			base.Body.Say(ThirteenHardBrynBoss.KillAttackChat[num], 1, 1000);
		...
		{
			int num = base.Game.Random.Next(0, ThirteenHardBrynBoss.ShootChat.Length);
			base.Body.Say(ThirteenHardBrynBoss.ShootChat[num], 1, 0);
		...
		private void MovingPlayer()
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				player.StartSpeedMult(x, player.Y , 200);
			}
		}
		...
		public override void OnKillPlayerSay()
		{
			...
			int num = base.Game.Random.Next(0, ThirteenHardBrynBoss.KillPlayerChat.Length);
			base.Body.Say(ThirteenHardBrynBoss.KillPlayerChat[num], 1, 0, 2000);
		}

		public override void OnShootedSay()
		{
			int num = base.Game.Random.Next(0, ThirteenHardBrynBoss.ShootedChat.Length);
			if (this.isSay == 0 && base.Body.IsLiving)
			{
				base.Body.Say(ThirteenHardBrynBoss.ShootedChat[num], 1, 900, 0);
				this.isSay = 1;
			}
			if (!base.Body.IsLiving)
			{
				num = base.Game.Random.Next(0, ThirteenHardBrynBoss.DiedChat.Length);
				base.Body.Say(ThirteenHardBrynBoss.DiedChat[num], 1, 100, 2000);
			}
		}
	}
}
