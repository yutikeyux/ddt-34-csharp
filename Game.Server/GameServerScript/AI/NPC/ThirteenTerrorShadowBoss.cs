...
namespace GameServerScript.AI.NPC
{
	public class ThirteenTerrorShadowBoss : ABrain
	{
		...
		public override void OnStartAttacking()
		{
			...
			{
				if (current.IsLiving && current.X > 1830)
				{
		...
		{
			this.m_moive = ((PVEGame)base.Game).Createlayer(1100, base.Body.Y, "moive", "asset.game.ten.tedabiaoji", "out", 1, 0);
		}
		...
		{
			int num = base.Game.Random.Next(0, ThirteenTerrorShadowBoss.AngryChat.Length);
			base.Body.Say(ThirteenTerrorShadowBoss.AngryChat[num], 1, 0);
		...
		{
			base.Body.CurrentDamagePlus = 5f;
			int num = base.Game.Random.Next(0, ThirteenTerrorShadowBoss.KillAttackChat.Length);
			((SimpleBoss)base.Body).Say(ThirteenTerrorShadowBoss.KillAttackChat[num], 1, 500);
