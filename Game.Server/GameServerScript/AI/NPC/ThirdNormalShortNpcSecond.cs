...
namespace GameServerScript.AI.NPC
{
	public class ThirdNormalShortNpcSecond : ABrain
	{
		...
		public override void OnBeginNewTurn()
		{
			...
			{
				string oneChat = ThirdNormalShortNpcSecond.GetOneChat();
		...
		{
			int num = ThirdNormalShortNpcSecond.random.Next(0, ThirdNormalShortNpcSecond.listChat.Length);
			return ThirdNormalShortNpcSecond.listChat[num];
		}
		...
		public static void LivingSay(List<Living> livings)
		{
			if (livings != null && livings.Count != 0)
			{
				...
				if (count <= 5)
				{
					num = ThirdNormalShortNpcSecond.random.Next(0, 2);
				}
				else if (count > 5 && count <= 10)
				{
					num = ThirdNormalShortNpcSecond.random.Next(1, 3);
				}
				else
				{
					num = ThirdNormalShortNpcSecond.random.Next(1, 4);
				}
				if (num > 0)
				{
