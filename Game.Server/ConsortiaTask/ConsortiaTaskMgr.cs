                ...
                lock (Consortiums)
                {
                    foreach (var consortia in Consortiums.Where(consortia => consortia != null).ToList())
                    {
                        ...
                        else
                        {
                            ...
                            int addrich = consortia.Task.Riches * 3 / 2;
                            if (consortiaBussiness.ConsortiaRichAdd(consortia.ID, ref addrich))
                            {
                                // Add zero-check before division
                                if (consortia.Task.Points <= 0)
                                {
                                    if (Log.IsErrorEnabled)
                                        Log.Error($"Consortia {consortia.ID} has invalid task points: {consortia.Task.Points}");
                                    RemoveConsortiaForm(consortia);
                                    continue;
                                }
                                ...
                                consortia.Players.ForEach(t =>
                                {
                                    ...
                                    t.SendMessage(eMessageType.ChatNormal, @"Birlik Ödülleri:");

                                    var content = $"Katılım: \n" +
                                                  $"{taskExpirience} Tecrübe." +
                                                  $"{taskPoints} Servet";

                                    t.SendMailToUser(new PlayerBussiness(), content, "Birlik Misyonu",
        ...
        private static List<ConsortiaTaskConditionInfo> CreateTaskConditions(int level, int taskId)
        {
            ...
            try
            {
                ...
                for (var i = 0; i < taskType.Count; i++)
                {
                    ...
                    // Check if we got a valid task condition
                    if (taskCondition == null)
                    {
                        if (Log.IsErrorEnabled)
                            Log.Error($"No task condition found for type {taskType[i]} and level {level}");
                        continue; // Skip this condition
                    }

                    // Validate required properties
                    if (taskCondition.Content == null || taskCondition.TargetCount <= 0)
                    {
                        if (Log.IsErrorEnabled)
                            Log.Error($"Invalid task condition data for type {taskType[i]} and level {level}");
                        continue; // Skip this condition
                    }
                ...
                // If we couldn't create any conditions, return empty list
                if (result.Count == 0)
                {
                    if (Log.IsErrorEnabled)
                        Log.Error($"Failed to create any task conditions for level {level}");
                    return new List<ConsortiaTaskConditionInfo>();
                }
            }
            catch (Exception e)
            {
