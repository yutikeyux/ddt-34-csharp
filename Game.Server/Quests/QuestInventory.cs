...
namespace Game.Server.Quests
{
    public class QuestInventory
    {
        ...
        public void ResetDailyQuest()
        {
            ...
            {
                List<QuestConditionInfo> questConditions = QuestMgr.GetQuestCondiction(item.Info);
                if (item.Info.CanRepeat)
                {
                    //if (item.Info.QuestID != 2)
                    //{
                    if (item.Data.IsComplete)
                    {
                        if (questConditions.Count > 0)
                        {
                            item.Data.Condition1 = questConditions[0].Para2;
                        }
                        if (questConditions.Count > 1)
                        {
                            item.Data.Condition2 = questConditions[1].Para2;
                        }
                        if (questConditions.Count > 2)
                        {
                            item.Data.Condition3 = questConditions[2].Para2;
                        }
                        if (questConditions.Count > 3)
                        {
                            item.Data.Condition4 = questConditions[3].Para2;
                        }
                        //if (item.Data.RepeatFinish > 0)
                        //{
                        //    --item.Data.RepeatFinish;
                        //    item.Data.IsComplete = false;
                        //}
                        //else if (item.Data.RepeatFinish == 0)
                        //{
                        //    item.Data.RepeatFinish = item.Info.RepeatMax;
                        //    item.Data.IsComplete = false;
                        //}
                        if (item.Data.RepeatFinish >= 0)
                        {
                        ...
                        {
                            log.Error("ResetDailyQuest RepeatFinish: " + item.Data.RepeatFinish);
                        }
                        item.Reset(m_player);
                        item.Update();
                        SaveToDatabase();
