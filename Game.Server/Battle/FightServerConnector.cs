...
namespace Game.Server.Battle
{
    public class FightServerConnector : BaseConnector
    {
        ...
        public void SendReport(GamePlayer player, string msg, bool isCommand)
        {
            GSPacketIn gSPacketIn = new GSPacketIn(1501, player.CurrentRoom.Game.Id);
            gSPacketIn.WriteInt(player.PlayerCharacter.ID);
            gSPacketIn.WriteString(msg);
            gSPacketIn.WriteBoolean(isCommand);
            SendTCP(gSPacketIn);
        }
    }
}
