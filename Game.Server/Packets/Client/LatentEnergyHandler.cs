...
namespace Game.Server.Packets.Client
{
    ...
    public class LatentEnergyHandler : IPacketHandler
    {
        ...
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            {
                _ = client.Out.SendMessage(eMessageType.Normal, LanguageMgr.GetTranslation("LatentEnergyHandler.Msg1"));
                return 0;
            }
            PlayerInventory inventory = client.Player.GetInventory((eBageType)BagType);
            string msg = "Potansiyel Başarılı!";
            GSPacketIn pkg = new((byte)ePackageType.LATENT_ENERGY, client.Player.PlayerCharacter.ID);
            if (type == 1)
            {
                ...
                {
                    _ = client.Out.SendMessage(eMessageType.Normal, "Yetersiz ürün.");
                    return 0;
                }
                ...
                _ = storeBag.RemoveCountFromStack(itemCell, 1);
            }
            else
            {
                equipCell.latentEnergyCurStr = equipCell.latentEnergyNewStr;
                msg = "Potansiyel değerleri başarıyla güncellendi!";
            }
