    ...
    public class PropBuyHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            if (PropItemMgr.PropFightBag.ToList().Contains(shopItemInfoById.TemplateID))
            {
                ...
                _ = ShopMgr.SetItemType(shopItemInfoById, num, ref damageScore, ref petScore, ref iTemplateID, ref iCount, ref gold, ref money, ref offer, ref gifttoken, ref medal, ref hardCurrency, ref LeagueMoney, ref medal, ref honor);
                if (itemTemplateInfo.CategoryID == 10)
                {
                    ...
                    int Money = playerCharacter.Money;
                    int MoneyLock = playerCharacter.MoneyLock;

                    if (gold <= playerCharacter.Gold && (money <= ((Money >= 0) ? Money : 0) || money <= ((MoneyLock >= 0) ? MoneyLock : 0)) && offer <= playerCharacter.Offer && gifttoken <= playerCharacter.GiftToken && medal <= playerCharacter.medal)
                    {
                        ...
                        if (client.Player.FightBag.AddItem(itemInfo, 0))
                        {
                            ...
                            _ = client.Player.MoneyDirect(money, true, false, true);
                            _ = client.Player.RemoveOffer(offer);
                            _ = client.Player.RemoveGiftToken(gifttoken);
                            _ = client.Player.RemoveMedal(medal);
                            _ = client.Player.RemovemyHonor(honor);
                        }
                    }
                    else
                    {
                        _ = client.Out.SendMessage(eMessageType.BIGBUGLE_NOTICE, LanguageMgr.GetTranslation("PropBuyHandler.NoMoney"));
                    }
                }
            }
            return 0;
