    ...
    public class GameTakeTempItemsHandler : IPacketHandler
    {
        private bool GetItem(GamePlayer player, ItemInfo item, ref string message)
        {
            if (item != null)
            {
                if (item.Template.BagType == eBageType.Card)
                {
                    ...
                    {
                        _ = player.TempBag.RemoveItem(item);
                    }
                    return true;
                }
                ...
                if (player.SendItemsToMail(items, "Çanta Dolu! Ürünü İade Et", "Administrator", eMailType.ItemOverdue))
                {
                    foreach (ItemInfo item2 in items)
                    {
                        _ = player.TempBag.RemoveItem(item2);
                    }
                }
            }
            return false;
        ...
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            if (num != -1)
            {
                ...
                _ = GetItem(client.Player, itemAt, ref message);
            }
            else
            {
            ...
            {
                _ = client.Out.SendMessage(eMessageType.BIGBUGLE_NOTICE, message);
            }
            return 0;
        }
    }
