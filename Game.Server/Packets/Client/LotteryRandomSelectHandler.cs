    ...
    public class LotteryRandomSelectHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            if (client.Player.Lottery >= 0 && client.Player.LotteryID > 0)
            {
                ...
                {
                    client.Player.SendMessage("Rulet hakkın bitti.");
                }
                else if (client.Player.PropBag.GetItemCount(templateId) < client.Player.Lottery + 1)
                {
                    client.Player.SendMessage("Yeterli anahtarın yok.");
                }
                else
                {
                    client.Player.Lottery++;
                    List<ItemInfo> list = [];
                    SpecialItemDataInfo specialInfo = new();
                    _ = ItemBoxMgr.CreateItemBox(client.Player.LotteryID, client.Player.LotteryItems, list, specialInfo);
                    if (list.Count > 0)
                    {
                        client.Player.LotteryAwardList.Add(list[0]);
                        GSPacketIn gSPacketIn = new(30);
                        ...
                        _ = client.Player.RemoveTemplate(templateId, client.Player.Lottery);
                    ...
                    {
                        client.Player.SendMessage("Ödül verileri alınırken bir hata oluştu.");
                    }
                }
            }
            else
            {
                client.Player.SendMessage("Rulet Spini başlatmadın!");
            }
            return 0;
        }
    }
