...
namespace Game.Server.Packets.Client
{
	...
	public class LotteryOpenBoxHandler : IPacketHandler
	{
		public int HandlePacket(GameClient client, GSPacketIn packet)
		{
			...
			{
				client.Out.SendMessage(eMessageType.GM_NOTICE, "Malezya Ruleti!");
				return 1;
			}
			...
			{
				client.Out.SendMessage(eMessageType.GM_NOTICE, "Rulet Slotları doldu ve tekrar açılması mümkün değildir.!");
			...
			if (itemByTemplateID2 != null && itemByTemplateID2.Count >= 1)
			{
				...
				if (list.Count > 0)
				{
					...
					else
					{
						...
						{
							client.Out.SendMessage(eMessageType.GM_NOTICE, itemByTemplateID.Template.Name + " yetersiz.");
							client.Out.SendTCP(GetAwards(list_item, string_, 4));
							return 0;
						}
				...
				{
					client.Out.SendMessage(eMessageType.GM_NOTICE, "Kazanılan: " + stringBuilder.ToString());
				}
				return 1;
			}
			list_item = client.Player.CaddyBag.GetItems();
