...
namespace Game.Server.Packets.Client
{
    ...
    public class LotteryOpenBoxHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            {
                _ = client.Out.SendMessage(eMessageType.GM_NOTICE, "Malezya Ruleti!");
                return 1;
            }
            int bageType = packet.ReadByte();
            int slot = packet.ReadInt();
            int num = packet.ReadInt();
            List<ItemInfo> list_item = [];
            ...
            {
                _ = client.Out.SendMessage(eMessageType.GM_NOTICE, "Rulet Slotları doldu ve tekrar açılması mümkün değildir.!");
            ...
            if (itemByTemplateID2 != null && itemByTemplateID2.Count >= 1)
            {
                ...
                SpecialItemDataInfo specialItemDataInfo = new();
                ...
                {
                    _ = stringBuilder.Append(specialItemDataInfo.Gold + LanguageMgr.GetTranslation("OpenUpArkHandler.Gold"));
                    _ = client.Player.AddGold(specialItemDataInfo.Gold);
                }
                if (specialItemDataInfo.GiftToken != 0)
                {
                    _ = stringBuilder.Append(specialItemDataInfo.GiftToken + LanguageMgr.GetTranslation("OpenUpArkHandler.GiftToken"));
                    _ = client.Player.AddGiftToken(specialItemDataInfo.GiftToken);
                }
                if (list.Count > 0)
                {
                    ItemInfo itemInfo = list[0];
                    if (num is not 112047 and not 112100 and not 112101)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            _ = client.Out.SendMessage(eMessageType.GM_NOTICE, itemByTemplateID.Template.Name + " yetersiz.");
                            client.Out.SendTCP(GetAwards(list_item, string_, 4));
                            return 0;
                        }
                        _ = client.Player.RemoveTemplate(templateId, num2);
                        _ = client.Player.AddBadLuckCaddy(1);
                    }
                    _ = inventory.AddItem(itemInfo);
                    client.Player.OnUsingItem(num, 1);
                    _ = stringBuilder.Append(itemInfo.Template.Name);
                    _ = client.Player.RemoveTemplate(num, 1);
                }
                ...
                {
                    _ = client.Out.SendMessage(eMessageType.GM_NOTICE, "Kazanılan: " + stringBuilder.ToString());
                }
                return 1;
            }
            list_item = client.Player.CaddyBag.GetItems();
        ...
        {
            GSPacketIn gSPacketIn = new(245);
            gSPacketIn.WriteBoolean((list_item.Count > 0));
        ...
        private void AlternateLotteryOpen(GamePlayer m_player, int lotteryId)
        {
            ...
            GSPacketIn gSPacketIn = new(29, m_player.PlayerCharacter.ID);
        ...
        public void OpenUpItem(string data, List<ItemInfo> infos, SpecialItemDataInfo specialInfo)
        {
            ...
            {
                _ = ItemBoxMgr.CreateItemBox(Convert.ToInt32(data), infos, specialInfo);
            }
        }
    }
}
