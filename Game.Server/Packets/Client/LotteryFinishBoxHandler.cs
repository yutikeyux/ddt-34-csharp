        ...
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            for (int i = 0; i < caddyBag.Capalility; i++)
            {
                ...
                if (itemAt != null)
                {
                    ...
                    _ = caddyBag.TakeOutItem(itemAt);
                }
            }
            if (list.Count > 0)
            {
                _ = client.Player.SendItemsToMail(list, "İade edilen öğeler", "Hayal Ruleti", eMailType.BuyItem);
            }
            if (client.Player.Lottery != -1 && client.Player.LotteryAwardList.Count > 0)
            {
                ...
                if (list2.Count > 0)
                {
                    _ = client.Player.SendItemsToMail(list2, "Çantanızın dolu olması nedeniyle öğeler iade edildi.", "Hayal Ruleti Ödülü", eMailType.BuyItem);
