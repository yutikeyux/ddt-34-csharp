...
namespace Game.Server.Packets.Client
{
    ...
    public class DailyAwardHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            switch (num)
            {
                case 0:
                    if (AwardMgr.AddDailyAward(client.Player))
                    {
                        ...
                        {
                            _ = stringBuilder.Append(LanguageMgr.GetTranslation("GameUserDailyAward.Success"));
                        }
                        else
                        {
                            _ = stringBuilder.Append(LanguageMgr.GetTranslation("GameUserDailyAward.Fail"));
                        }
                    }
                    else
                    {
                        _ = stringBuilder.Append(LanguageMgr.GetTranslation("GameUserDailyAward.Fail1"));
                    }
                    break;
                case 2:
                    {
                        ...
                        {
                            _ = stringBuilder.Append($"Bugün zaten 1 kez aldınız!");
                            break;
                        }
                        _ = AwardMgr.AddEggAward(client.Player);
                        ItemInfo cloneItem = ItemInfo.CreateFromTemplate(ItemMgr.FindItemTemplate(112059), 1, 113);
                        _ = client.Player.PropBag.AddTemplate(cloneItem, 1);
                        _ = stringBuilder.Append(LanguageMgr.GetTranslation("Günlük Sürpriz Yumurta Alımı Başarılı!"));
                        _ = LanguageMgr.GetTranslation("GameServer.DailyEgg.Notice.Msg", client.Player.PlayerCharacter.NickName, 112059);
                ...
                case 3:
                    if (client.Player.PlayerCharacter.CanTakeVipReward)
                    {
                        ...
                        _ = playerBussiness2.UpdateLastVIPPackTime(client.Player.PlayerCharacter.ID);
                    }
                    else
                    {
                        _ = stringBuilder.Append("Bugün ödülünü aldın!");
                    }
                    break;
                case 5:
                    {
                        ...
                        _ = playerBussiness.UpdateDailyLogList(dailyLogListInfo);
                        break;
                    }
            }
            if (point != 0)
            {
                _ = stringBuilder.Append(point + LanguageMgr.GetTranslation("OpenUpArkHandler.Money"));
                _ = client.Player.AddMoney(point);
            }
            if (gold != 0)
            {
                _ = stringBuilder.Append(gold + LanguageMgr.GetTranslation("OpenUpArkHandler.Gold"));
                _ = client.Player.AddGold(gold);
            }
            if (giftToken != 0)
            {
                _ = stringBuilder.Append(giftToken + LanguageMgr.GetTranslation("OpenUpArkHandler.GiftToken"));
                _ = client.Player.AddGiftToken(giftToken);
            }
            if (medal != 0)
            {
                _ = stringBuilder.Append(medal + LanguageMgr.GetTranslation("OpenUpArkHandler.Medal"));
                _ = client.Player.AddMedal(medal);
            }
            StringBuilder stringBuilder2 = new();
            foreach (ItemInfo item in list)
            {
                _ = stringBuilder2.Append(item.Template.Name + "x" + item.Count + ",");
                if (!client.Player.AddTemplate(item, item.Template.BagType, item.Count, eGameView.RouletteTypeGet))
                {
                    using PlayerBussiness playerBussiness4 = new();
                    item.UserID = 0;
                    _ = playerBussiness4.AddGoods(item);
                    MailInfo mail = new()
                    {
                    ...
                    _ = playerBussiness4.SendMail(mail);
                    text = LanguageMgr.GetTranslation("OpenUpArkHandler.Mail");
                }
            }
            if (stringBuilder2.Length > 0)
            {
                _ = stringBuilder2.Remove(stringBuilder2.Length - 1, 1);
            ...
            _ = client.Out.SendMessage(eMessageType.GM_NOTICE, text + stringBuilder.ToString());
