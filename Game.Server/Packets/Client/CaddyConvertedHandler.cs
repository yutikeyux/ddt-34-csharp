...
namespace Game.Server.Packets.Client
{
    ...
    public class CaddyConvertedHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            bool flag = false;
            int riches = 1000;
            string text = "ConsortiaRichiUpdateHandler.Failed";
            ...
            using (PlayerBussiness playerBussiness = new())
            {
                ...
                foreach (ConsortiaUserInfo consortiaUserInfo in array2)
                {
                    ...
                    if (consortiaUserInfo.UserID != client.Player.PlayerCharacter.ID && playerBussiness.SendMail(mailInfo))
                    {
                        ...
                        {
                            _ = client.Player.Out.SendMailResponse(consortiaUserInfo.UserID, eMailRespose.Receiver);
                        }
            ...
            if (flag)
            {
                using ConsortiaBussiness consortiaBussiness = new();
                _ = consortiaBussiness.ConsortiaRichRemove(client.Player.PlayerCharacter.ConsortiaID, ref riches);
            }
            _ = client.Out.SendConsortiaMail(flag, client.Player.PlayerCharacter.ID);
