...
namespace Game.Server.Packets.Client
{
    ...
    internal class MarryApplyHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            {
                _ = client.Player.Out.SendMessage(eMessageType.ChatNormal, LanguageMgr.GetTranslation("MarryApplyHandler.Msg2"));
                return 1;
            }
            ...
            if (itemByTemplateID == null)
            {
                ...
                {
                    _ = client.Player.Out.SendMessage(eMessageType.GM_NOTICE, LanguageMgr.GetTranslation("MarryApplyHandler.Msg6"));
                    return 1;
                }
                if (!client.Player.MoneyDirect(shopItemInfo.AValue1, IsAntiMult: false, false, false))
                {
                    return 1;
                }
                flag = false;
            }
            MarryApplyInfo info = new()
            {
                UserID = num,
            ...
            if (playerBussiness.SavePlayerMarryNotice(info, 0, ref id))
            {
                if (flag)
                {
                    _ = client.Player.RemoveItem(itemByTemplateID);
                }
                else
                {
                    _ = ShopMgr.FindShopbyTemplatID(11103).FirstOrDefault();
                }
                _ = client.Player.Out.SendPlayerMarryApply(client.Player, client.Player.PlayerCharacter.ID, client.Player.PlayerCharacter.NickName, loveProclamation, id);
                GameServer.Instance.LoginServer.SendUpdatePlayerMarriedStates(num);
                _ = userSingleByUserID.NickName;
                _ = client.Player.Out.SendMessage(eMessageType.ChatNormal, LanguageMgr.GetTranslation("MarryApplyHandler.Msg3"));
            }
            return 0;
        }
    }
