...
namespace Game.Server.Packets.Client
{
    ...
    public class MailGetAttachHandler : IPacketHandler
    {
        ...
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            GSPacketIn pkg = new((byte)ePackageType.GET_MAIL_ATTACHMENT, client.Player.PlayerCharacter.ID);
            using PlayerBussiness db = new();
            ...
            if (mes != null)
            {
                ...
                if (mes.Type > 100 && !client.Player.MoneyDirect(oldMoney, IsAntiMult: true, true, false))
                {
                    return 0;
                }
                ...
                if (db.UpdateMail(mes, oldMoney))
                {
                    ...
                    {
                        _ = client.Out.SendMailResponse(mes.SenderID, eMailRespose.Receiver);
                        _ = client.Out.SendMailResponse(mes.ReceiverID, eMailRespose.Send);
                    }
                    _ = player.AddMoney(money);
                    _ = player.AddGold(gold);
                    _ = player.AddGiftToken(giftToken);
                    foreach (string item in annex)
                    {
                        _ = GetAnnex(item, client.Player, ref msg, ref result, ref eMsg);
                    }
                }
                pkg.WriteInt(id);
                ...
                _ = client.Out.SendMessage(eMsg, string.IsNullOrEmpty(msg) ? LanguageMgr.GetTranslation("MailGetAttachHandler.Success") : msg);
            }
            else
            {
                _ = client.Out.SendMessage(eMessageType.BIGBUGLE_NOTICE, LanguageMgr.GetTranslation("MailGetAttachHandler.Falied"));
            }
            return 0;
        }
    }
