        ...
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            while (true)
            {
                if (num2 < num)
                {
                    ...
                    {
                        client.Player.SendMessage("Yenilenemiyo");
                    }
                    else
                    {
                        ...
                        if (itemAt != null && itemAt.ValidDate != 0)
                        {
                            ...
                            if (gold <= client.Player.PlayerCharacter.Gold && (money <= myMoney || money <= myMoneyLock) && offer <= client.Player.PlayerCharacter.Offer && gifttoken <= client.Player.PlayerCharacter.GiftToken && petScore <= client.Player.PlayerCharacter.petScore && Score <= client.Player.PlayerCharacter.Score)
                            {
                                // ---------------------------------------------------------
                                ...
                                {
                                    // ÖDEME BAŞARISIZSA (LİMİT DOLU) - Süre uzatma işlemini kesip iptal ediyoruz.
                                    itemAt.ValidDate = validDate;
                                    itemAt.Count = count;
                                    client.Out.SendMessage(eMessageType.BIGBUGLE_NOTICE, "Süre yenileme başarısız! Günlük kupon harcama limitinizi aşmış olabilirsiniz.");
                                }
                        ...
                        {
                            client.Player.SendMessage("Bu ürün yenilenemez");
                        }
                    }
                    num2++;
                    continue;
            ...
            client.Player.SendMessage("Dolandırıcı mısın la??? neyse sistem yutiye bildirdi...");
            return 0;
        }
    }
}