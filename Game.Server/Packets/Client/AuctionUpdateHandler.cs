...
namespace Game.Server.Packets.Client
{
	...
	public class AuctionUpdateHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
			...
			{
				client.Out.SendMessage(eMessageType.GM_NOTICE, "Sizin bu işlemi gerçekleştirme yetkiniz yok.");
				return 0;
			}
			if (client.Player.PlayerCharacter.Grade < num3)
			{
				client.Out.SendMessage(eMessageType.GM_NOTICE, string.Format("Bu işlemi gerçekleştirmek için {0} seviye gereklidir!.", num3));
				return 0;
			}
