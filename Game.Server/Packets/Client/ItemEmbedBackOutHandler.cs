...
namespace Game.Server.Packets.Client
{
    ...
    public class ItemEmbedBackOutHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            if (client.Player.PlayerCharacter.Money < needMoney && client.Player.PlayerCharacter.MoneyLock < needMoney && !client.Player.isPlayerWarrior())
            {
                _ = client.Out.SendMessage(eMessageType.BIGBUGLE_NOTICE, LanguageMgr.GetTranslation("ItemComposeHandler.NoMoney"));
                return 0;
            }
            if (client.Player.PropBag.CountTotalEmptySlot() <= 0)
            {
                _ = client.Out.SendMessage(eMessageType.GM_NOTICE, LanguageMgr.GetTranslation("UserChangeItemPlaceHandler.full"));
                return 0;
            }
            ...
            if (result)
            {
                ...
                if (!client.Player.AddItem(gem))
                {
                    ...
                    int type = 8;
                    _ = player.SendItemsToMail(items, content, title, (eMailType)type);
                }
                ...
                _ = client.Out.SendMessage(eMessageType.GM_NOTICE, LanguageMgr.GetTranslation("OK"));
            }
            else
            {
                pkg.WriteInt(1);
