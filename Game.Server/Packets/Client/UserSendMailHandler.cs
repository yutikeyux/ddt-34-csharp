...
namespace Game.Server.Packets.Client
{
    ...
    public class UserSendMailHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            {
                _ = client.Out.SendMessage(eMessageType.GM_NOTICE, LanguageMgr.GetTranslation("UserSendMailHandler.GoldNotEnought"));
            ...
            {
                _ = client.Out.SendMessage(eMessageType.GM_NOTICE, "Hesabınız negatif ve e-posta gönderemiyor.");
                packet.WriteBoolean(val: false);
                client.Out.SendTCP(packet);
                return 1;
            }
            ...
            {
                _ = client.Out.SendMessage(eMessageType.GM_NOTICE, LanguageMgr.GetTranslation("UserSendMailHandler.IsLimitMail"));
                packet.WriteBoolean(val: false);
                client.Out.SendTCP(packet);
                return 1;
            }
            ...
            {
                _ = client.Out.SendMessage(eMessageType.GM_NOTICE, "Hesabın bu işlevi gerçekleştirme izni yok.");
                return 0;
            }
            ...
            using (PlayerBussiness pb = new())
            {
                ...
                if (user != null && !string.IsNullOrEmpty(nickName))
                {
                    if (user.NickName != client.Player.PlayerCharacter.NickName)
                    {
                        ...
                        List<ItemInfo> item = [];
                        List<eBageType> bagType = [];
                        StringBuilder annexRemark = new();
                        _ = annexRemark.Append(LanguageMgr.GetTranslation("UserSendMailHandler.AnnexRemark"));
                        int index = 0;
                        if (place != -1)
                        {
                            ...
                            if (info != null && !info.IsBinds)
                            {
                                ...
                                index++;
                                _ = annexRemark.Append(index);
                                _ = annexRemark.Append("、");
                                _ = annexRemark.Append(message.Annex1Name);
                                _ = annexRemark.Append("x");
                                _ = annexRemark.Append(info.Count);
                                _ = annexRemark.Append(";");
                            }
                        }
                        if (num4 != -1)
                        {
                            ...
                            if (info != null && !info.IsBinds)
                            {
                                ...
                                _ = annexRemark.Append(index);
                                _ = annexRemark.Append("、");
                                _ = annexRemark.Append(message.Annex2Name);
                                _ = annexRemark.Append("x");
                                _ = annexRemark.Append(info.Count);
                                _ = annexRemark.Append(";");
                            }
                        ...
                        if (num5 != -1)
                        {
                            ...
                            if (info != null && !info.IsBinds)
                            {
                                ...
                                _ = annexRemark.Append(index);
                                _ = annexRemark.Append("、");
                                _ = annexRemark.Append(message.Annex3Name);
                                _ = annexRemark.Append("x");
                                _ = annexRemark.Append(info.Count);
                                _ = annexRemark.Append(";");
                            }
                        ...
                        if (num6 != -1)
                        {
                            ...
                            if (info != null && !info.IsBinds)
                            {
                                ...
                                _ = annexRemark.Append(index);
                                _ = annexRemark.Append("、");
                                _ = annexRemark.Append(message.Annex4Name);
                                _ = annexRemark.Append("x");
                                _ = annexRemark.Append(info.Count);
                                _ = annexRemark.Append(";");
                            }
                        ...
                        if (isPay)
                        {
                            ...
                            if (money > 0)
                            {
                                ...
                                _ = annexRemark.Append(index);
                                _ = annexRemark.Append("、");
                                _ = annexRemark.Append(LanguageMgr.GetTranslation("UserSendMailHandler.PayMoney"));
                                _ = annexRemark.Append(money);
                                _ = annexRemark.Append(";");
                            }
                        }
                        else
                        {
                            ...
                            {
                                message.Money = money;
                                _ = client.Player.RemoveMoneyNoviceActive(money);
                                //client.RemoveMoney(money, true);
                                index++;
                                _ = annexRemark.Append(index);
                                _ = annexRemark.Append("、");
                                _ = annexRemark.Append(LanguageMgr.GetTranslation("UserSendMailHandler.Money"));
                                _ = annexRemark.Append(money);
                                _ = annexRemark.Append(";");
                            }
                        }
                        ...
                        {
                            _ = client.Player.RemoveGold(100);
                            for (int i = 0; i < item.Count; i++)
                            {
                                item[i].UserID = 0;
                                _ = client.Player.RemoveItem(item[i]);
                                item[i].IsExist = true;
                            }
                        }
                        _ = client.Player.SaveIntoDatabase();
                        ...
                        if (user.State != 0)
                        {
                            _ = client.Player.Out.SendMailResponse(user.ID, eMailRespose.Receiver);
                        }
                        _ = client.Player.Out.SendMailResponse(client.Player.PlayerCharacter.ID, eMailRespose.Send);
                    }
                    else
                    {
