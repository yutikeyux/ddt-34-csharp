...
namespace Game.Server.Packets.Client
{
    ...
    public class UserGetBoxHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            LoadUserBoxInfo loadUserBoxInfo;
            if (num == 0)
            {
                ...
                if (loadUserBoxInfo != null && num3 >= num2 && loadUserBoxInfo.Condition == num2)
                {
                    ...
                    _ = playerBussiness.UpdateBoxProgression(player.PlayerCharacter.ID, player.PlayerCharacter.BoxProgression, player.PlayerCharacter.GetBoxLevel, player.PlayerCharacter.AddGPLastDate, DateTime.Now, num2);
            ...
            if (flag)
            {
                if (loadUserBoxInfo != null)
                {
                    ...
                    int exp = 0;
                    int honor = 0;
                    _ = ItemBoxMgr.CreateItemBox(Convert.ToInt32(loadUserBoxInfo.TemplateID), list2, ref gold, ref point, ref giftToken, ref exp, ref honor);
                    if (gold > 0)
                    {
                        _ = player.AddGold(gold);
                    }
                    if (point > 0)
                    {
                        _ = player.AddMoney(point);
                    }
                    if (giftToken > 0)
                    {
                        _ = player.AddGiftToken(giftToken);
                    }
                    if (exp > 0)
                    {
                        _ = player.AddGP(exp, false);
                    }
                    if (honor > 0)
                    {
                        _ = player.AddHonor(honor);
                    }
                    ...
                    if (num4 == 0)
                    {
                        ...
                        if (loadUserBoxInfo != null)
                        {
                            _ = player.Out.SendMessage(eMessageType.GM_NOTICE, $"Zaman sandığı alma başarılı! Bir sonraki sandığı {loadUserBoxInfo.Condition} dakika sonra alabilirsiniz!");
                        }
                        else
                        {
                            _ = player.Out.SendMessage(eMessageType.GM_NOTICE, $"Bugün zaten tüm sandıkları kazandınız!");
                        }
                    }
                    else
                    {
                        ...
                        if (loadUserBoxInfo != null)
                        {
                            _ = player.Out.SendMessage(eMessageType.GM_NOTICE, LanguageMgr.GetTranslation("UserGetTimeBoxHandler.level", loadUserBoxInfo.Level));
                        }
                        else
                        {
                            _ = player.Out.SendMessage(eMessageType.GM_NOTICE, LanguageMgr.GetTranslation("UserGetTimeBoxHandler.over"));
                        }
                    }
                    ...
                    {
                        _ = player.Out.SendMessage(eMessageType.GM_NOTICE, LanguageMgr.GetTranslation("UserGetTimeBixHandler.full"));
                        val = true;
                        _ = player.Out.SendMailResponse(player.PlayerCharacter.ID, eMailRespose.Receiver);
                    }
                }
                else
                {
            ...
            {
                _ = player.Out.SendMessage(eMessageType.GM_NOTICE, LanguageMgr.GetTranslation("UserGetTimeBoxHandler.fail"));
            }
            if (num4 == 0)
            {
