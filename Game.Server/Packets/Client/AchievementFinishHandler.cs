...
namespace Game.Server.Packets.Client
{
    ...
    public class AchievementFinishHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            {
                GSPacketIn gSPacketIn = new(230, client.Player.PlayerCharacter.ID);
                gSPacketIn.WriteInt(num);
                DateTime now = DateTime.Now;
                gSPacketIn.WriteInt(now.Year);
                gSPacketIn.WriteInt(now.Month);
                gSPacketIn.WriteInt(now.Day);
                _ = client.Player.AchievementInventory.AddAchievementData(achievementInfo);
                _ = client.Player.AchievementInventory.SendReward(achievementInfo);
