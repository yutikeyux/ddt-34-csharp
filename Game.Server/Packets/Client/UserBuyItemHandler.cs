    ...
    public class UserBuyItemHandler : IPacketHandler
    {
        ...
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            if (count > 0 && count <= 99)
            {
                ...
                if (shopItem != null)
                {
                    if (shopItem.LimitCount != -1)
                    {
                        if (shopItem.LimitCount == 0 || buyitems.Count > shopItem.LimitCount)
                        {
                            client.Out.SendMessage(eMessageType.BIGBUGLE_NOTICE, LanguageMgr.GetTranslation("UserBuyItemHandler.LimitCount1", shopItem.LimitCount));
                            return 1;
                        }
                    }
                }
                if (gold >= 0 && money >= 0 && offer >= 0 && gifttoken >= 0 && petScore >= 0 && Score >= 0 && dmgScore >= 0 && (gold > 0 || money > 0 || offer > 0 || gifttoken > 0 || petScore > 0 || Score > 0 || dmgScore > 0 || needitemsinfo.Count > 0))
                {
                    ...
                    if (gold <= client.Player.PlayerCharacter.Gold && (money <= myMoney || money <= myMoneyLock) && offer <= client.Player.PlayerCharacter.Offer && gifttoken <= client.Player.PlayerCharacter.GiftToken && petScore <= client.Player.PlayerCharacter.petScore && Score <= client.Player.PlayerCharacter.Score && dmgScore <= client.Player.PlayerCharacter.damageScores)
                    {
                        ...
                        string str = $"money: {money} | gold: {gold} | offter: {offer} | gifttoken: {gifttoken} | petScore: {petScore} | boguScore: {Score} | moneyBefore: {moneyBefore} | moneyAfter: {moneyAfter} | dmg Score: {dmgScore} | moneyLockBefore: {moneyLockBefore} | moneyLockAfter: {moneyLockAfter}";
