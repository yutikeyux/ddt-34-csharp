    ...
    internal class MarryRoomInfoUpdateHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            if (client.Player.CurrentMarryRoom != null && client.Player.PlayerCharacter.ID == client.Player.CurrentMarryRoom.Info.PlayerID)
            {
                ...
                {
                    _ = playerBussiness.UpdateMarryRoomInfo(currentMarryRoom.Info);
                }
                _ = currentMarryRoom.SendMarryRoomInfoUpdateToScenePlayers(currentMarryRoom);
                _ = client.Player.Out.SendMessage(eMessageType.GM_NOTICE, LanguageMgr.GetTranslation("MarryRoomInfoUpdateHandler.Successed"));
                return 0;
            }
            return 1;
        }
