    ...
    public class ArmorPiercerEquipEffect : BasePlayerEffect
    {
        ...
        public override bool Start(Living living)
        {
            ...
            if (effect != null)
            {
                ...
                effect.m_count = ((m_count > effect.m_count) ? m_count : effect.m_count);
                return true;
            }
        ...
        {
            player.PlayerShoot -= ChangeProperty;      // Düzeltildi: += yerine -=
            player.AfterPlayerShooted -= player_AfterPlayerShooted;
        }
        ...
        {
            IsTrigger = false;
            if (!player.CurrentBall.IsSpecial() && rand.Next(100) < m_probability && player.AttackGemLimit == 0)  // Düzeltildi: tekrar eden kontrol kaldırıldı
            {
                player.AttackGemLimit = m_count;       // Düzeltildi: sabit 5 yerine m_count kullanıldı
                player.FlyingPartical = 0;
