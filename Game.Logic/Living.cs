...
public class Living : Physics
{
    ...
    {
        get { return m_lastBlood; }
        set { m_lastBlood = value; }
    }
    ...
    protected int MakeDamage(Living target) 
    {
        ...
        {
            int addPoint = (int)getHertAddition((target as Player).DeputyWeapon);
        ...
        double num4 = 0.95 * (baseGuard - (double)(3 * Grade)) / (500.0 + baseGuard - (double)(3 * Grade));
        double num5 = ((!(defence - Lucky < 0.0)) ? (0.95 * (defence - Lucky) / (600.0 + defence - Lucky)) : 0.0);
    ...
    public bool ShootImp(int bombId, int x, int y, int force, int angle, int bombCount, int shootCount)
    {
        ...
        GSPacketIn gSPacketIn = new GSPacketIn((byte)ePackageTypeLogic.GAME_CMD, base.Id);
    ...
    {
        if ((Config.IsHelper && (this is SimpleNpc || this is SimpleBoss) && source is Player) || (PetEffects != null && PetEffects.ActiveNoDamage))
        {
            return false;
        }
        bool result = false;
