...
namespace Game.Logic
{
	public class BaseGame : AbstractGame
	{
		...
		{
			GSPacketIn pkg = new GSPacketIn((byte)ePackageTypeLogic.GAME_CMD, living_0.Id)
			{
		...
		{
			GSPacketIn pkg = new GSPacketIn((byte)ePackageTypeLogic.GAME_CMD, id);
			pkg.Parameter1 = id;
			pkg.WriteByte(223);
			pkg.WriteInt(id);
		...
		{
			GSPacketIn pkg = new GSPacketIn((byte)ePackageTypeLogic.GAME_CMD, player_0.Id)
			{
		...
		{
			GSPacketIn pkg = new GSPacketIn((byte)ePackageTypeLogic.GAME_CMD, living.Id);
			pkg.Parameter1 = living.Id;
			pkg.WriteByte(72);
		...
		{
			GSPacketIn gSPacketIn = new GSPacketIn((byte)ePackageTypeLogic.GAME_CMD, living_0.Id);
			gSPacketIn.Parameter1 = living_0.Id;
		...
		{
			GSPacketIn gSPacketIn = new GSPacketIn((byte)ePackageTypeLogic.GAME_CMD, player.Id);
		...
		public bool SetMap(int mapId)
		{
			if (GameState != eGameState.Playing)
			{
				...
				{
					Console.WriteLine("Cant find MapID: " + mapId);
				}
			}
			return false;
		}
		...
		internal void method_0(Player player)
		{
			GSPacketIn pkg = new GSPacketIn((short)91);
			pkg.WriteByte((byte)157);
		...
		public List<Living> GetNPCLivings()
		{
			List<Living> list = new List<Living>();
