...
namespace Game.Logic
{
    public class PVPGame : BaseGame
    {
        ...
        private static readonly double EVEN_SERVER_XU = 2;

        private static readonly double EVEN_SERVER_GP = 2;
        ...
        private int CalculateGuildMatchResult(List<Player> players, int winTeam)
        {
            ...
            foreach (var p2 in players.Where(p => p.Team == winTeam))
            {
            ...
            int riches = (int)(float)(((winTeam == 1) ? m_blueTeam.Count : m_redTeam.Count) + teamTotalHurt / 2000f);
        ...
        public void GameOver()
        {
            ...
            pkg.WriteInt(tieStatus);//_loc_3 = _loc_2.readInt(); tieStatus
            ...
            foreach (Player p in players)
            {
                ...
                {
                    int moneyPVP = base.Random.Next(PVPGame.MONEY_MIN_RATE_LOSE, PVPGame.MONEY_MAX_RATE_LOSE);
                    int money = base.Random.Next(PVPGame.MONEY_MIN_RATE_LOSE, PVPGame.MONEY_MAX_RATE_LOSE);
                    int expPVP = 0;
                    int giftPVP = 0;
                    int timex2 = 1;
                ...
                if (m_roomType == eRoomType.ConsortiaBattle)
                {
                    p.PlayerDetail.UpdateConsortiaBattle(p.Blood, isWin, tieStatus);
                }
        ...
        public void AddShadow(Player fp)
        {
            AddLiving(fp);
            fp.StartMoving();
            this.method_0(fp);
