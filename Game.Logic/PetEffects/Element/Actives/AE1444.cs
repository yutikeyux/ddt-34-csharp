...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1444 : BasePetEffect
    {
        ...
        {
            AE1444 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1444) as AE1444;
            if (effect != null)
            {
        ...
        private void Player_BeforeTakeDamage(Living living, Living source, ref int damageAmount, ref int criticalAmount)
        {
            ...
            {
                m_added = 500;
                m_added += (damageAmount + criticalAmount) * 20 / 100;
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                IsTrigger = true;
                //player.AddPetEffect(new CE1444(1, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
