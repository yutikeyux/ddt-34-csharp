...
{
    public class AE1344 : BasePetEffect
    {
        ...
        public AE1344(int count, int probability, int type, int skillId, int delay, string elementID)
            : base(ePetEffectType.AE1344, elementID)
        {
            m_count = count;
            m_coldDown = count;
        ...
        {
            AE1344 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1344) as AE1344;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                m_added = (int)(player.Lucky * 10 / 100);
                player.Lucky += m_added;
