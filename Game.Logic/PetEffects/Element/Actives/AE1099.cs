...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1099 : BasePetEffect
    {
        ...
        {
            if (living.PetEffectList.GetOfType(ePetEffectType.AE1099) is AE1099 effect)
            {
        ...
        private void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                {
                    //ally.Game.SendPetBuff(ally, ElementInfo, true, 1);
                    ally.AddPetEffect(new CE1099(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
