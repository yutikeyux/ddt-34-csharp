...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1221 : BasePetEffect
    {
        ...
        {
            if (living.PetEffectList.GetOfType(ePetEffectType.AE1221) is AE1221 effect)
            {
        ...
        private void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                foreach (Player enemy in enemys)
                {
                    ...
                    enemy.AddPetEffect(new CE1221(2, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
