...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1426 : BasePetEffect
    {
        ...
        {
            if (living.PetEffectList.GetOfType(ePetEffectType.AE1426) is AE1426 effect)
            {
        ...
        private void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                {
                    enemy.AddPetEffect(new CE1426(1, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
                }
