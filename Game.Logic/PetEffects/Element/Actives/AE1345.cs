...
{
    public class AE1345 : BasePetEffect
    {
        ...
        public AE1345(int count, int probability, int type, int skillId, int delay, string elementID)
            : base(ePetEffectType.AE1345, elementID)
        {
            m_count = count;
            m_coldDown = count;
        ...
        {
            AE1345 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1345) as AE1345;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId)
            {
                ...
                m_added = (int)(player.Lucky * 15 / 100);
                player.Lucky += m_added;
