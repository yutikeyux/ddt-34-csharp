...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1152 : BasePetEffect
    {
        ...
        {
            if (living.PetEffectList.GetOfType(ePetEffectType.AE1152) is AE1152 effect)
            {
        ...
        private void Player_AfterKillingLiving(Living living, Living target, int damageAmount, int criticalAmount)
        {
            if (IsTrigger)
            {
                ...
                target.AddPetEffect(new CE1152(3, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString(), living), 0);
            }
        }
