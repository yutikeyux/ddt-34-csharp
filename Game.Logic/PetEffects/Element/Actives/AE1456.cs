...
namespace Game.Logic.PetEffects.Element.Actives
{
    public class AE1456 : BasePetEffect
    {
        ...
        {
            AE1456 effect = living.PetEffectList.GetOfType(ePetEffectType.AE1456) as AE1456;
            if (effect != null)
            {
        ...
        void player_AfterBuffSkillPetByLiving(Player player)
        {
            if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
            {
                ...
                player.AddBlood(player.MaxBlood / 100 * 15);
                player.SyncAtTime = false;
                player.AddPetEffect(new CE1456(1, m_probability, m_type, m_currentId, m_delay, ElementInfo.ID.ToString()), 0);
