...
namespace Game.Logic
{
    public class PVEGame : BaseGame
    {
        ...
        private int CalculatePlayerExperience(Player player)
        {
            if (TotalKillCount == 0)
            {
                return 1;
            ...
            double gradeDifference = Math.Abs((double)player.Grade - TotalNpcGrade / (double)TotalKillCount);

            if (gradeDifference >= 7.0)
            {
                return 1;
            }
            ...
            if (TotalKillCount > 0)
            {
                playerContribution += (double)player.TotalKill / (double)TotalKillCount * 0.4;
            }

            if (TotalHurt > 0)
            {
                playerContribution += (double)player.TotalHurt / (double)TotalHurt * 0.4;
            }

            if (player.IsLiving)
            {
                playerContribution += 0.4;
            }

            double gradeMultiplier = 1.0;

            if (gradeDifference >= 3.0 && gradeDifference <= 4.0)
            {
                gradeMultiplier = 0.7;
            }
            else if (gradeDifference >= 5.0 && gradeDifference <= 6.0)
            {
                gradeMultiplier = 0.4;
            }

            double playerExperienceRatio = (0.9 + (double)(BeginPlayersCount - 1) * 0.4) / (double)PlayerCount;

            double playerExperience = TotalNpcExperience * playerContribution * gradeMultiplier * playerExperienceRatio;
            playerExperience = (playerExperience == 0.0) ? 1.0 : playerExperience;

            return (int)playerExperience;
        }
        ...
        {
            if (GameState != eGameState.Playing && GameState != eGameState.PrepareGameOver)
            {
                return;
            }
            ...
            {
                BaseGame.log.ErrorFormat("game ai script {0} error:{1}", GameState, ex);
            }
            ...
            if (GameType == eGameType.FightLab && IsWin)
            {
                foreach (Player player in allFightPlayers)
                {
                    player.PlayerDetail.SetFightLabPermission(m_info.ID, m_hardLevel, MissionInfo.Id);
                }
            }

            GSPacketIn packet = new GSPacketIn((byte)ePackageTypeLogic.GAME_CMD);
            packet.WriteByte((byte)eTankCmdType.MISSION_OVE);
            packet.WriteInt(BossCardCount);
            ...
            else
            {
                ...
                packet.WriteString($"show{1 + SessionId}.jpg");
                packet.WriteBoolean(true);
            }
            ...
            foreach (Player player in allFightPlayers)
            {
                ...
                {
                    player.PlayerDetail.SetPvePermission(m_info.ID, getNextHardLevel(m_hardLevel));
                }

                player.OnPlayerClearBuffSkillPet();
                player.EffectList.StopAllEffect();

                if (!IsWin && IsLabyrinth())
                {
                    player.PlayerDetail.OutLabyrinth(IsWin);
                }

                int experience = CalculatePlayerExperience(player);

                if (player.FightBuffers.ConsortionAddPercentGoldOrGP > 0)
                {
                    experience += experience * player.FightBuffers.ConsortionAddPercentGoldOrGP / 100;
                }

                int score = CalculateScore(player);
                m_missionAI.CalculateScoreGrade(player.TotalAllScore);

                player.CanTakeOut = m_bossCardCount;

                if (player.CurrentIsHitTarget)
                {
                    player.TotalHitTargetCount++;
                }

                CalculateHitRate(player.TotalHitTargetCount, player.TotalShootCount);
                ...
                player.GainGP = player.PlayerDetail.AddGP(experience);
                player.TotalAllExperience += player.GainGP;
                player.TotalAllScore += score;
                player.BossCardCount = m_bossCardCount;
                packet.WriteInt(player.PlayerDetail.PlayerCharacter.ID);
                packet.WriteInt(player.PlayerDetail.PlayerCharacter.Grade);
                packet.WriteInt(0);
                packet.WriteInt((player.GainGP > 10000) ? 10000 : player.GainGP);
                packet.WriteBoolean(IsWin);
                packet.WriteInt(BossCardCount);
                packet.WriteInt(player.BossCardCount);
                packet.WriteBoolean(false);
                packet.WriteBoolean(false);
            }
            if (BossCardCount > 0)
            {
                packet.WriteInt(m_gameOverResources.Count);
                foreach (string gameOverResource in m_gameOverResources)
                {
                    packet.WriteString(gameOverResource);
                }
            }
            SendToAll(packet);
            StringBuilder teamRed = new StringBuilder();
            foreach (Player item in allFightPlayers)
            {
                teamRed.Append(item.PlayerDetail.PlayerCharacter.ID).Append(",");
                item.Ready = false;
                item.PlayerDetail.OnMissionOver(item.Game, IsWin, MissionInfo.Id, item.TurnNum);
            }
            int winTeam = (IsWin ? 1 : 2);
            string teamA = teamRed.ToString();
            string teamB = "";
            string playResult = "";
            ...
            if (IsWin && IsBossWar != "")
            {
                teamBlue.Append(IsBossWar).Append(",");
                foreach (Player item4 in allFightPlayers)
                {
                    teamBlue.Append("PlayerCharacter ID: ").Append(item4.PlayerDetail.PlayerCharacter.ID).Append(",");
                    teamBlue.Append("Grade: ").Append(item4.PlayerDetail.PlayerCharacter.Grade).Append(",");
                    teamBlue.Append("TurnNum): ").Append(item4.TurnNum).Append(",");
                    teamBlue.Append("Attack: ").Append(item4.PlayerDetail.PlayerCharacter.Attack).Append(",");
                    teamBlue.Append("Defence: ").Append(item4.PlayerDetail.PlayerCharacter.Defence).Append(",");
                    teamBlue.Append("Agility: ").Append(item4.PlayerDetail.PlayerCharacter.Agility).Append(",");
                    teamBlue.Append("Luck: ").Append(item4.PlayerDetail.PlayerCharacter.Luck).Append(",");
                    teamBlue.Append("BaseAttack: ").Append(item4.PlayerDetail.GetBaseAttack()).Append(",");
                    teamBlue.Append("MaxBlood: ").Append(item4.MaxBlood).Append(",");
                    teamBlue.Append("BaseDefence: ").Append(item4.PlayerDetail.GetBaseDefence()).Append(",");
                    if (item4.PlayerDetail.SecondWeapon != null)
                    {
                        teamBlue.Append("SecondWeapon TemplateID: ").Append(item4.PlayerDetail.SecondWeapon.TemplateID).Append(",");
                        teamBlue.Append("SecondWeapon StrengthenLevel: ").Append(item4.PlayerDetail.SecondWeapon.StrengthenLevel).Append(".");
                    }
                }
            }
            BossWarField = teamBlue.ToString();
        ...
        public void GameOverMovie()
        {
            ...
            if (!val)
            {
                ...
                foreach (Player player2 in allFightPlayers)
                {
                    ...
                    int gp = CalculatePlayerExperience(player2);
